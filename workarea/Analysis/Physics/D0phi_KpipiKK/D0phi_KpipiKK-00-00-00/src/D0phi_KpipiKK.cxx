...
/// `D0phi_KpipiKK`), and if required a documentation string. Note that you should define the
/// paramters themselves in the header (D0phi_KpipiKK/D0phi_KpipiKK.h) and that you should assign
/// the values in `share/D0phi_KpipiKK.txt`.
...
StatusCode D0phi_KpipiKK::initialize_rest()
{
  ...
  fNTuple_topology.AddItem<double>("mD0");  /// <li> `"mD0"`:  Invariant mass of the \f$D^0\f$
                                            /// candidate in the \f$J/\psi \to D^0\phi\f$.
  fNTuple_topology.AddItem<double>("mphi"); /// <li> `"mphi"`: Invariant mass of the \f$\phi\f$
                                            /// candidate in the \f$J/\psi \to D^0\phi\f$.
  fNTuple_topology.AddItem<double>(
    "pD0"); /// <li> `"pD0"`:  Momentum of the \f$D^0\f$ candidate in the \f$J/\psi \to D^0\phi\f$.
  fNTuple_topology.AddItem<double>("pphi"); /// <li> `"pphi"`: Momentum of the \f$\phi\f$ candidate
                                            /// in the \f$J/\psi \to D^0\phi\f$.
                                            /// </ol>
...
StatusCode D0phi_KpipiKK::execute_rest()
{
    ...
    { /// The particle identification first decides whether the track is more likely to have come
      ...
      if(fCut_PIDProb.FailsMin(fPIDInstance->probPion()))
        continue; /// A cut is then applied on whether the probability to be a pion (or kaon) is at
                  /// least `fCut_PIDProb_min` (see eventual settings in `D0phi_KpipiKK.txt`).
  ...
  for(fMcPhoton1Iter = fMcPhotons.begin(); fMcPhoton1Iter != fMcPhotons.end(); ++fMcPhoton1Iter)
    for(fMcPhoton2Iter = fMcPhotons.begin(); fMcPhoton2Iter != fMcPhotons.end(); ++fMcPhoton2Iter)
      for(fMcKaonNeg1Iter = fMcKaonNeg.begin(); fMcKaonNeg1Iter != fMcKaonNeg.end();
          ++fMcKaonNeg1Iter)
        for(fMcKaonNeg2Iter = fMcKaonNeg.begin(); fMcKaonNeg2Iter != fMcKaonNeg.end();
            ++fMcKaonNeg2Iter)
          for(fMcKaonPosIter = fMcKaonPos.begin(); fMcKaonPosIter != fMcKaonPos.end();
              ++fMcKaonPosIter)
                ...
                ++fMcPionPosIter)
            {
              ...
              /// from MC truth.
              KKFitResult_D0phi_KpipiKK fitresult(*fMcKaonNeg1Iter, *fMcKaonNeg2Iter,
                                                  *fMcKaonPosIter, *fMcPionPosIter, *fMcPhoton1Iter,
                                                  *fMcPhoton2Iter);
