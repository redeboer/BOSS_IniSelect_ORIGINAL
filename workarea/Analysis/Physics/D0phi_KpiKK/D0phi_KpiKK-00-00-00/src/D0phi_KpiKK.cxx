...
/// `D0phi_KpiKK`), and if required a documentation string. Note that you should define the
/// paramters themselves in the header (D0phi_KpiKK/D0phi_KpiKK.h) and that you should assign the
/// values in `share/D0phi_KpiKK.txt`.
                        ...
                        "Number of events that had exactly 2 K-, 1 K+ and 1 pi+ PID tracks")
{
  ...
  PostConstructor();
  fCreateChargedCollection =
    true; /// @remark Set `fCreateChargedCollection` to `true` to ensure that the preselection of
          /// charged tracks is made. The neutral tracks are not necessary.
...
StatusCode D0phi_KpiKK::execute_rest()
{
    ...
    for(fMcKaonNeg2Iter = fMcKaonNeg.begin(); fMcKaonNeg2Iter != fMcKaonNeg.end();
      ...
      for(fMcKaonPosIter = fMcKaonPos.begin(); fMcKaonPosIter != fMcKaonPos.end(); ++fMcKaonPosIter)
            ...
            ++fMcPionPosIter)
        {
          ...
          /// truth.
          KKFitResult_D0phi_KpiKK fitresult(*fMcKaonNeg1Iter, *fMcKaonNeg2Iter, *fMcKaonPosIter,
                                            *fMcPionPosIter);
  ...
  /// `D0phi_KpiKK::MeasureForBestFit` for the definition of this measure.
