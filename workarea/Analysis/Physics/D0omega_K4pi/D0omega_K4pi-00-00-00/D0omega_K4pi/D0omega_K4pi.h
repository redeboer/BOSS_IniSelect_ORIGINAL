...
/// @{
	...
	protected:
		/// @name Track collections and iterators
			...
			std::vector<Event::McParticle*>::iterator fMcKaonNegIter;  ///< Iterator for looping over the MC collection of negative kaons.
			...
			std::vector<Event::McParticle*>::iterator fMcPionNegIter;  ///< Iterator for looping over the MC collection of positive pions.
			std::vector<Event::McParticle*>::iterator fMcPionPos1Iter; ///< Iterator for looping over the MC collection of positive pions (1st occurence).
			std::vector<Event::McParticle*>::iterator fMcPionPos2Iter; ///< Iterator for looping over the MC collection of positive pions (2st occurence).
			...
			std::vector<EvtRecTrack*>::iterator fKaonNegIter;  ///< Iterator for looping over the collection of negative kaons.
			...
			std::vector<EvtRecTrack*>::iterator fPionPos1Iter; ///< Iterator for looping over the collection of positive pions (1st occurence).
			std::vector<EvtRecTrack*>::iterator fPionPos2Iter; ///< Iterator for looping over the collection of positive pions (2st occurence).
			///@}
		...
		/// @name NTuples (eventual TTrees)
			...
			NTupleContainer fNTuple_fit4c_all;  ///< `NTuple::Tuple` container for the 4-constraint fit branch containing *all* combinations.
			NTupleContainer fNTuple_fit4c_best; ///< `NTuple::Tuple` container for the 4-constraint fit branch containing only the *best* combination.
		...
		/// @name Counters and cut objects
			///@{
			CutObject fCutFlow_NChargedOK;    ///< **Cut flow counter**: total number of events that have exactly the number of charged tracks we want.
			CutObject fCutFlow_NFitOK;        ///< **Cut flow counter**: total number of events where there is at least one combination where the kinematic fit worked.
			CutObject fCutFlow_NPIDnumberOK;  ///< **Cut flow counter**: total number of events that that has exactly the identified tracks that we want.
			CutObject fCutFlow_NetChargeOK;  ///< **Cut flow counter**: total number of events where the measured netto charge was \f$0\f$. This cut is used to exclude events where some charged tracks were not detected (an \f$e^+e^-\f$ collision has \f$0\f$ net charge).
			CutObject fCut_GammaAngle; ///< Cut on angle between the photon and the nearest charged track *in degrees*.
			CutObject fCut_GammaPhi;   ///< Cut on \f$\phi\f$ angle between the photon and the nearest charged track *in radians*.
			CutObject fCut_GammaTheta; ///< Cut on \f$\theta\f$ angle between the photon and the nearest charged track *in radians*.
			///@}
