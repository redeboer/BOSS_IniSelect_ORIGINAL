...
// * ------- LIBRARIES ------- * //
// * ========================= * //
#include "D0omega_Kenu3pi/D0omega_Kenu3pi.h"
...
// * ------- CONSTRUCTOR ------- * //
// * =========================== * //
/// Constructor for the `D0omega_Kenu3pi` algorithm.
/// Here, you should declare properties: give them a name, assign a parameter (data member of `D0omega_Kenu3pi`), and if required a documentation string. Note that you should define the paramters themselves in the header (D0omega_Kenu3pi/D0omega_Kenu3pi.h) and that you should assign the values in `share/jopOptions_D0omega_Kenu3pi.txt`. Algorithms should inherit from Gaudi's `Algorithm` class. See https://dayabay.bnl.gov/dox/GaudiKernel/html/classAlgorithm.html.
D0omega_Kenu3pi::D0omega_Kenu3pi(const string& name, ISvcLocator* pSvcLocator) :
...
StatusCode D0omega_Kenu3pi::initialize()
{
...
StatusCode D0omega_Kenu3pi::execute()
{
  /// <ol>
  ...
  */
  ...
  Vint        iep, ipip, ipim, iKm; // vector of indices of good tracks
  Vp4         pep, ppip, ppim, pKm; // vector of momenta
  ...
  for(int i = 0; i < nGood; ++i)
  {
    ...
    if(pid->probPion() > pid->probKaon() && pid->probPion() > pid->probElectron())
    {
    ...
    {
      if(pid->probKaon() > pid->probElectron())
      {
  ...
  WTrackParameter wvepTrk(mK, epTrk->getZHelix(), epTrk->getZError());
  ...
  WTrackParameter wpim = vtxfit->wtrk(2);
  ...
  if(fDo_fit4c)
  {
    ...
    for(int i = 0; i < nGam - 1; ++i)
    {
      ...
      for(int j = i + 1; j < nGam; j++)
      {
        ...
        kkmfit->init();
        kkmfit->AddTrack(0, wep);
        kkmfit->AddTrack(1, wKm);
        kkmfit->AddTrack(2, wpim);
        kkmfit->AddTrack(3, wpip);
        kkmfit->AddTrack(4, 0., g1Trk);
        kkmfit->AddTrack(5, 0., g2Trk);
        kkmfit->AddMissTrack(6, 0.);
...
StatusCode D0omega_Kenu3pi::finalize()
{
  MsgStream log(msgSvc(), name());
