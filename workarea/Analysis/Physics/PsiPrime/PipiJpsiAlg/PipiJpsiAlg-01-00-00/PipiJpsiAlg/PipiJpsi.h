...
/// Modified header of the stock BOSS package `PipiJpsi` (version `00-00-03`). Analyse \f$ \psi'
/// \rightarrow J/\psi \pi \pi \f$ and \f$ J/\psi \rightarrow \mathrm{di\-leptons} \f$ events. This
/// example package particularly teaches the use of Monte Carlo truth.
...
private:
  ...
  /// values for these cuts should be set in the `PipiJpsi::PipiJpsi` constructor (see `.cxx` file)
  ///@{
  ...
  double
    m_distin_emuon; ///< Value of \f$E/|\vec{p}|\f$ that is used to distinguish electron versus muon
  ...
  /// PipiJpsi::initialize() step. This is also the place where you name these variables, so make
  ...
  NTuple::Item<double>
                       m_dthe; ///< \f$\theta\f$ angle difference with nearest charged track (degrees)
  ...
  NTuple::Array<double>
    m_cos_theta; ///< \f$\cos(\theta)\f$ of the lorentz vector in the cylindrical coordinate system
  NTuple::Array<double>
                         m_phi; ///< \f$\phi\f$ angle of the lorentz vector in the cylindrical coordinate system
  NTuple::Matrix<double> m_four_mom; ///< \f$E\f$, \f$p_x\f$, \f$p_y\f$, and \f$p_z\f$ for each
                                     ///< lepton/pion (hence a \f$4\times4\f$ matrix)
                                     // * Invariant masses * //
  ...
  NTuple::Item<double>
    m_mass_recoil; ///< Invariant mass of the lab 4-momentum minus the 4-momenta of the two pions
  NTuple::Item<double>
    m_inv_mass; ///< Invariant mass of the total system (\f$J/\psi\f$ plus the two pions)
  ...
  NTuple::Item<long>
    m_event_flag; ///< Characterisation of the event: 3 or 4 tracks, 4=>4 tracks, 0=> miss pi+, 1=>
                  ///< miss pi-, 2=> miss lepton+, 3=> miss lepton-
  NTuple::Item<double>
    m_trans_ratio_lepm; ///< Ratio of the 4-momentum axial distance (\f$\rho\f$ in the cylindrical
                        ///< coordinate system) versus \f$|\vec{p}|\f$ for the negative lepton.
  NTuple::Item<double>
    m_trans_ratio_lepp; ///< Ratio of the 4-momentum axial distance (\f$\rho\f$ in the cylindrical
                        ///< coordinate system) versus \f$|\vec{p}|\f$ for the positive lepton.
  NTuple::Item<double>
    m_trans_ratio_pionm; ///< Ratio of the 4-momentum axial distance (\f$\rho\f$ in the cylindrical
                         ///< coordinate system) versus \f$|\vec{p}|\f$ for the negative pion.
  NTuple::Item<double>
    m_trans_ratio_pionp; ///< Ratio of the 4-momentum axial distance (\f$\rho\f$ in the cylindrical
                         ///< coordinate system) versus \f$|\vec{p}|\f$ for the positive pion.
                         // * Indices * //
  NTuple::Item<long> m_run;          ///< Run number
  NTuple::Item<long> m_event;        ///< Event number
  NTuple::Item<long> m_index;        ///< For indexed item. Ranges from `0` to `3`: for \f$\pi^+\f$,
                                     ///< \f$\pi^-\f$, \f$l^+\f$, and \f$l^-\f$
                                     // * Matches * //
  NTuple::Item<long> m_pion_matched; ///< Number of identified pions. We don't need smear, actually,
                                     ///< assume one track of pi pi is missing, the recoil mass is
                                     ///< actually the invariant of pair-lepton.
  NTuple::Item<long> m_lep_matched; ///< Number of identified leptons (actually non-pions). We don't
                                    ///< need smear, actually, assume one track of pi pi is missing,
                                    ///< the recoil mass is actually the invariant of pair-lepton.
                                    // * Monte Carlo truth * //
  NTuple::Item<long>
                       m_idxmc; ///< Total number of (useful) MC particles in this event (used for the arrays)
