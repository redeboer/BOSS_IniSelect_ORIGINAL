  ...
  vector<int> vMidx)
{
  ...
  {
    cerr << "Infor: The size of the vector for the inclusive or intermediate-resonance-allowed "
            "sequential decay branches is zero!"
         << endl
         << endl;
    return nCount;
  }
      ...
      for(unsigned int j = 1; j < incOrIRASeqDcyBrs.size(); j++)
      {
        for(unsigned int k = vVIDcyBr2WRTIDcyBr1[i][j - 1] + 1; k < dcyTr.size(); k++)
        {
          if(vIIncIncOrIRASeqDcyBrs[j] == 0)
          {
            ...
            {
              if(dcyTr[k] == incOrIRASeqDcyBrs[j] &&
                 (vMidxOfHead2[k] == vIdxOfHead2[k] ||
                  (vIIRAIncOrIRASeqDcyBrs[j - 1] == 0 &&
                   vMidxOfHead2[k] ==
                     vIdxOfHead2[(unsigned int)
                                   vVIDcyBr2WRTIDcyBr1[i][(unsigned int)vMIDcyBr1WRTIDcyBr1[j]]]) ||
                  (vIIRAIncOrIRASeqDcyBrs[j - 1] == 1 &&
                   isPaADescendantOfPb(
                     vMidx, vIdxOfHead2[k],
                     vIdxOfHead2[(unsigned int)
                                   vVIDcyBr2WRTIDcyBr1[i][(unsigned int)vMIDcyBr1WRTIDcyBr1[j]]]))))
            ...
            else
            {
              if((*(dcyTr[k].begin())) == (*(incOrIRASeqDcyBrs[j].begin())))
              {
                ...
                getSubsetsOfVPidandVMidxFromAP(subsetOfVPidFromAP, subsetOfVMidxFromAP,
                                               vIdxOfHead2[k]);
                if(countIRADcyBr(subsetOfVPidFromAP, subsetOfVMidxFromAP, incOrIRASeqDcyBrs[j],
                                 headsAreRequiredToBeMatched) &&
                   (vMidxOfHead2[k] == vIdxOfHead2[k] ||
                    (vIIRAIncOrIRASeqDcyBrs[j - 1] == 0 &&
                     vMidxOfHead2[k] == vIdxOfHead2[(unsigned int)vVIDcyBr2WRTIDcyBr1[i][(
                                          unsigned int)vMIDcyBr1WRTIDcyBr1[j]]]) ||
                    (vIIRAIncOrIRASeqDcyBrs[j - 1] == 1 &&
                     isPaADescendantOfPb(vMidx, vIdxOfHead2[k],
                                         vIdxOfHead2[(unsigned int)vVIDcyBr2WRTIDcyBr1[i][(
                                           unsigned int)vMIDcyBr1WRTIDcyBr1[j]]]))))
                {
                  vVIDcyBr2WRTIDcyBr1[i].push_back(k);
                  break;
          ...
          {
            if(isLiaMatchedWithLib(incOrIRASeqDcyBrs[j], dcyTr[k]) &&
               (vMidxOfHead2[k] == vIdxOfHead2[k] ||
                (vIIRAIncOrIRASeqDcyBrs[j - 1] == 0 &&
                 vMidxOfHead2[k] ==
                   vIdxOfHead2[(
