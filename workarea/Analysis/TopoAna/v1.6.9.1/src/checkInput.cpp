...
void topoana::checkInput()
{
  ...
  else
  {
    ...
    { cout << "  " << m_nmsOfIptRootFls[i] << endl; } cout << endl;
    ...
    if(hasWildcards == true)
    {
      ...
      for(unsigned int i = 0; i < m_nmsOfIptRootFls.size(); i++)
      { chn->Add(m_nmsOfIptRootFls[i].c_str()); } TObjArray* objArray = chn->GetListOfFiles();
      TIter                                                  next(objArray);
      ...
      {
        cout << endl
             << "In total, " << nCmltFls << " files match the names listed above." << endl
             << endl;
      }
      else
      {
  ...
  {
    cerr << "Error: The branch name of the array of the mother indeces of particles is empty!"
         << endl;
    exit(-1);
  }
  else
  {
    cout << "Branch name of the array of the mother indeces of particles: " << m_brNmOfMidx << endl
         << endl;
  }

  TChain* chn = new TChain(m_trNm.c_str());
  for(unsigned int i = 0; i < m_nmsOfIptRootFls.size(); i++)
  { chn->Add(m_nmsOfIptRootFls[i].c_str()); } bool allIptsAreOK = true;
  ...
  while((chnElmt = (TChainElement*)next()))
  {
    ...
    if(fl->IsZombie())
    {
      cerr << "Error: The input root file \"" << chnElmt->GetTitle() << "\" is zombie!" << endl
           << endl;
      allIptsAreOK = false;
    }
    else
    {
      ...
      if(!tr)
      {
        cerr << "Error: The input root file \"" << chnElmt->GetTitle()
             << "\" does not contain a tree named \"" << m_trNm << "\"!" << endl
             << endl;
        allIptsAreOK = false;
      }
      else
      {
        ...
        if(!br0)
        {
          cerr << "Error: The tree \"" << m_trNm << "\" in the input root file \""
               << chnElmt->GetTitle() << "\" does not contain a branch named \"" << m_brNmOfNps
               << "\"!" << endl
               << endl;
          allIptsAreOK = false;
        }
        ...
        {
          cerr << "Error: The tree \"" << m_trNm << "\" in the input root file \""
               << chnElmt->GetTitle() << "\" does not contain a branch named \"" << m_brNmOfPid
               << "\"!" << endl
               << endl;
          allIptsAreOK = false;
        }
        ...
        {
          cerr << "Error: The tree \"" << m_trNm << "\" in the input root file \""
               << chnElmt->GetTitle() << "\" does not contain a branch named \"" << m_brNmOfMidx
               << "\"!" << endl
               << endl;
          allIptsAreOK = false;
        }
  ...
  { cout << "Maximum number of entries to be processed: " << m_nEtrsMax << endl << endl; } else
  {
    cout << "Maximum number of entries to be processed: ULONG_MAX (default)" << endl << endl;
  }

  if(m_cut.empty()) { cout << "No cut is used to select entries." << endl << endl; }
  else
  {
    cout << "Cut to select entries: " << m_cut << endl << endl;
    if(m_cutMethod == true)
    {
      cout << "The method to apply cut to array variables: True as long as one instance is true."
           << endl
           << endl;
    }
    else
    {
      cout << "The method to apply cut to array variables: False as long as one instance is false "
              "(default)."
           << endl
           << endl;
    }
  ...
  if(m_ccSwitch == true)
  { cout << "Charge conjugate objects are processed together." << endl << endl; } else
  {
    cout << "Charge conjugate objects are not processed together (default)." << endl << endl;
  }

  if(m_ignoreGISR == true) { cout << "gISR photons are ignored (default)." << endl << endl; }
  else
  {
    cout << "gISR photons are not ignored." << endl << endl;
  }

  if(m_ignoreGFSR == true) { cout << "gFSR photons are ignored (default)." << endl << endl; }
  else
  {
  ...
  {
    cout << "Maximum hierarchy of heading decay branches to be processed in each event: "
         << m_hHdDcyBrsMax << endl
         << endl;
  }
  else
  {
    cout << "Maximum hierarchy of heading decay branches to be processed in each event: UINT_MAX "
            "(default)"
         << endl
         << endl;
  }
  ...
  if(m_vIdPid.size() == 0)
  { cout << "No particles are specified to ignore their decay." << endl << endl; } else
  {
    ...
    for(unsigned int i = 0; i < m_vIdPid.size(); i++)
    {
      ...
      for(unsigned int j = 0; j < i; j++)
      {
        if(m_vIdPid[i] == m_vIdPid[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi << " particle is same as the " << j + 1
               << ordNumSufj << " particle!" << endl;
        ...
        else if(m_ccSwitch == true && m_vIdICcPid[j] != 0 && m_vIdPid[i] == m_vIdCcPid[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " particle is same as the charge conjugate counterpart of " << j + 1 << ordNumSufj
               << " particle!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      ...
      if(m_ccSwitch == true)
      {
        ...
        if(m_vIdCcPid[i] == m_vIdPid[i])
          m_vIdICcPid.push_back(0);
        else
          m_vIdICcPid.push_back(1);
      }
    }
  ...
  else
  {
    ...
    for(unsigned int i = 0; i < m_vIddPid.size(); i++)
    {
      ...
      if(m_ccSwitch == true)
      {
        ...
        if(m_vIddCcPid[i] == m_vIddPid[i])
          m_vIddICcPid.push_back(0);
        else
          m_vIddICcPid.push_back(1);
      }
    }
  ...
  {
    cout << "Maximum number of decay final states to be printed: " << m_nDcyFStsToBePrtdMax << endl
         << endl;
  }
  else
  {
    cout << "Maximum number of decay final states to be printed: ULONG_MAX (default)" << endl
         << endl;
  }

  if(m_spr1stBrsOfDcyTrs == true)
  {
    cout << "The first branches of decay trees in the output txt/tex/pdf files are suppressed."
         << endl
         << endl;
  }
  else
  {
    cout << "The first branches of decay trees in the output txt/tex/pdf files are not suppressed. "
            "(default)."
         << endl
         << endl;
  }
  ...
  { cout << "The topology tags in the output tex/pdf files are suppressed." << endl << endl; } else
  {
    cout << "The topology tags in the output tex/pdf files are not suppressed. (default)." << endl
         << endl;
  }

  if(m_nEtrsMaxInASngOptRootFl != ULONG_MAX)
  {
    cout << "Maximum number of entries to be saved in a single output root file:"
         << m_nEtrsMaxInASngOptRootFl << endl
         << endl;
  }
  else
  {
    cout << "Upper limit of the total bytes of the cloned TTree object of " << m_trNm
         << ": 3 GB in memory (default)." << endl
         << endl;
  }

  if(m_nDcyTrsToBeAnlzdMax != ULONG_MAX)
  {
    cout << "Maximum number of decay trees to be analyzed: " << m_nDcyTrsToBeAnlzdMax << endl
         << endl;
    if(m_nEtrsOfCtrSmpMax != 100000)
    {
      cout << "Maximum number of entries in the control sample: " << m_nEtrsOfCtrSmpMax << endl
           << endl;
    }
    else
    {
  ...
  {
    cout << "The topology analysis of decay trees and decay final states are skipped." << endl
         << endl;
  }
  else
  {
    cout
      << "The topology analysis of decay trees and decay final states are not skipped. (default)."
      << endl
      << endl;
  }
  ...
  {
    cout << "No particles are specified for performing the topology analysis of the decay branches "
            "begun with them."
         << endl
         << endl;
  }
  else
  {
    ...
    for(unsigned int i = 0; i < m_vPttaPid.size(); i++)
    {
      ...
      if(m_pidTxtPnmMap[m_vPttaPid[i]].size() <= 6)
        cout << "\t\t\t";
      else if(m_pidTxtPnmMap[m_vPttaPid[i]].size() <= 14)
        cout << "\t\t";
      else if(m_pidTxtPnmMap[m_vPttaPid[i]].size() <= 22)
        cout << "\t";
      if(m_vNDcyBrsMax[i] != ULONG_MAX)
      {
        cout << "Maximum number of decay branches to be printed: " << m_vNDcyBrsMax[i] << endl
             << endl;
      }
      else
      {
    ...
    for(unsigned int i = 0; i < m_vPttaPid.size(); i++)
    {
      ...
      if(m_ccSwitch == true)
      {
        ...
        if(m_vPttaCcPid[i] == m_vPttaPid[i])
          m_vPttaICcPid.push_back(0);
        else
          m_vPttaICcPid.push_back(1);
  ...
  {
    cout << "No inclusive decay branches are specified for performing the topology analysis of the "
            "exclusive decay branches matched with them."
         << endl
         << endl;
  }
  else
  {
    cout << "Perform the topology analysis of the exclusive decay branches matched with the "
    ...
    for(unsigned int i = 0; i < m_vVPttaPid1.size(); i++)
    {
      ...
      {
        cout << "Maximum number of decay branches to be printed: " << m_vNDcyBrsMax1[i] << endl
             << endl;
      }
      else
      {
    ...
    m_vPttaIncDcyBr.clear();
    m_vPttaCcIncDcyBr.clear();
    ...
    m_vVIDcyBrCcIncDcyBr.clear();
    vector<int> vIDcyBrIncDcyBr, vIDcyBrCcIncDcyBr;
    vIDcyBrIncDcyBr.clear();
    vIDcyBrCcIncDcyBr.clear();
    m_vVNDcyBrIncDcyBr.clear();
    m_vVNDcyBrCcIncDcyBr.clear();
    vector<int> vNDcyBrIncDcyBr, vNDcyBrCcIncDcyBr;
    vNDcyBrIncDcyBr.clear();
    vNDcyBrCcIncDcyBr.clear();
    for(unsigned int i = 0; i < m_vVPttaPid1.size(); i++)
    {
      pttaIncDcyBr.clear();
      for(unsigned int j = 1; j < m_vVPttaPid1[i].size(); j++)
        pttaIncDcyBr.push_back(m_vVPttaPid1[i][j]);
      sortByPidAndPchrg(pttaIncDcyBr);
      if(m_vVPttaPid1[i][0] != m_pidOfISt) { pttaIncDcyBr.push_front(m_vVPttaPid1[i][0]); }
      else
      {
      ...
      for(unsigned int j = 0; j < m_vPttaIncDcyBr.size(); j++)
      {
        if(pttaIncDcyBr == m_vPttaIncDcyBr[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi << " inclusive decay is same as the "
               << j + 1 << ordNumSufj << " inclusive decay!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
        else if(m_ccSwitch == true && m_vPttaICcIncDcyBr[j] != 0 &&
                pttaIncDcyBr == m_vPttaCcIncDcyBr[j])
        {
          if(i == 0)
            ordNumSufi = "st";
          else if(i == 1)
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " inclusive decay is same as the charge conjugate counterpart of " << j + 1
               << ordNumSufj << " inclusive decay!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      }
      ...
      if(m_ccSwitch == true)
      {
        ...
        if((*liit) != -11) { pttaCcIncDcyBr.push_front(getCcPid((*liit))); }
        else
        {
        ...
        if(pttaCcIncDcyBr == pttaIncDcyBr)
          m_vPttaICcIncDcyBr.push_back(0);
        else
          m_vPttaICcIncDcyBr.push_back(1);
  ...
  if(m_vSigPid.size() == 0) { cout << "No signal particles are specified." << endl << endl; }
  else
  {
    ...
    for(unsigned int i = 0; i < m_vSigPid.size(); i++)
    {
      ...
      for(unsigned int j = 0; j < i; j++)
      {
        if(m_vSigPid[i] == m_vSigPid[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi << " signal particle is same as the "
               << j + 1 << ordNumSufj << " signal particle!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
        else if(m_ccSwitch == true && m_vICcSigP[j] != 0 && m_vSigPid[i] == m_vCcSigPid[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal particle is same as the charge conjugate counterpart of " << j + 1
               << ordNumSufj << " signal particle!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      }
      ...
      if(m_ccSwitch == true)
      {
        ...
        if(m_vCcSigPid[i] == m_vSigPid[i])
          m_vICcSigP.push_back(0);
        else
          m_vICcSigP.push_back(1);
  ...
  { cout << "No signal inclusive decay branches are specified." << endl << endl; } else
  {
    ...
    m_vSigIncDcyBr.clear();
    m_vCcSigIncDcyBr.clear();
    list<int> sigIncDcyBr, ccSigIncDcyBr;
    m_vISigIncDcyBr.clear();
    m_vICcSigIncDcyBr.clear();
    m_vNSigIncDcyBr.clear();
    m_vNCcSigIncDcyBr.clear();
    for(unsigned int i = 0; i < m_vVSigPid1.size(); i++)
    {
      sigIncDcyBr.clear();
      for(unsigned int j = 1; j < m_vVSigPid1[i].size(); j++)
        sigIncDcyBr.push_back(m_vVSigPid1[i][j]);
      sortByPidAndPchrg(sigIncDcyBr);
      if(m_vVSigPid1[i][0] != m_pidOfISt) { sigIncDcyBr.push_front(m_vVSigPid1[i][0]); }
      else
      {
      ...
      for(unsigned int j = 0; j < m_vSigIncDcyBr.size(); j++)
      {
        if(sigIncDcyBr == m_vSigIncDcyBr[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal inclusive decay branch is same as the " << j + 1 << ordNumSufj
               << " signal inclusive decay branch!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
        else if(m_ccSwitch == true && m_vICcSigIncDcyBr[j] != 0 &&
                sigIncDcyBr == m_vCcSigIncDcyBr[j])
        {
          if(i == 0)
            ordNumSufi = "st";
          else if(i == 1)
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal inclusive decay branch is same as the charge conjugate counterpart of "
               << j + 1 << ordNumSufj << " signal inclusive decay branch!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      }
      ...
      if(m_ccSwitch == true)
      {
        ...
        if((*liit) != -11) { ccSigIncDcyBr.push_front(getCcPid((*liit))); }
        else
        {
        ...
        if(ccSigIncDcyBr == sigIncDcyBr)
          m_vICcSigIncDcyBr.push_back(0);
        else
          m_vICcSigIncDcyBr.push_back(1);
  ...
  { cout << "No signal sequential decay branches are specified." << endl << endl; } else
  {
    ...
    for(unsigned int i = 0; i < m_vVSigPid2.size(); i++)
    {
      for(unsigned int j = 0; j < m_vVSigPid2[i].size(); j++)
      {
        ...
        if(m_pidTxtPnmMap[m_vVSigPid2[i][j]].size() < 7)
          cout << "\t\t\t";
        else if(m_pidTxtPnmMap[m_vVSigPid2[i][j]].size() < 15)
          cout << "\t\t";
        else
          cout << "\t";
        cout << setiosflags(ios::right) << setw(3) << m_vVSigMidx2[i][j]
    ...
    m_vVSigSeqDcyBrsMidxOfHead.clear();
    m_vVCcSigSeqDcyBrsMidxOfHead.clear();
    vector<int> vSigSeqDcyBrsMidxOfHead, vCcSigSeqDcyBrsMidxOfHead;
    m_vISigSeqDcyBrs.clear();
    m_vICcSigSeqDcyBrs.clear();
    m_vNSigSeqDcyBrs.clear();
    m_vNCcSigSeqDcyBrs.clear();
    ...
    for(unsigned int i = 0; i < m_vVSigPid2.size(); i++)
    {
      ...
      getDcyTr(m_vVSigPid2[i], m_vVSigMidx2[i], sigSeqDcyBrs, vSigSeqDcyBrsIdxOfHead,
               vSigSeqDcyBrsMidxOfHead);
      // remove the branch from e+ and e- to the first particle if all the following particles are
      // descendant s of the first particle.
      ...
      for(unsigned int j = 0; j < m_vSigSeqDcyBrs.size(); j++)
      {
        if(sigSeqDcyBrs == m_vSigSeqDcyBrs[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal sequential decay branches is same as the " << j + 1 << ordNumSufj
               << " signal sequential decay branches!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
        else if(m_ccSwitch == true && m_vICcSigSeqDcyBrs[j] != 0 &&
                sigSeqDcyBrs == m_vCcSigSeqDcyBrs[j])
        {
          if(i == 0)
            ordNumSufi = "st";
          else if(i == 1)
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr
            << "Error: The " << i + 1 << ordNumSufi
            << " signal sequential decay branches is same as the charge conjugate counterpart of "
            << j + 1 << ordNumSufj << " signal sequential decay branches!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      }
      ...
      if(m_ccSwitch == true)
      {
        ...
        vCcSigMidx2.clear();
        for(unsigned int j = 0; j < m_vVSigPid2[i].size(); j++)
          vCcSigPid2.push_back(getCcPid(m_vVSigPid2[i][j]));
        ...
        getDcyTr(vCcSigPid2, vCcSigMidx2, ccSigSeqDcyBrs, vCcSigSeqDcyBrsIdxOfHead,
                 vCcSigSeqDcyBrsMidxOfHead);
        // remove the branch from e+ and e- to the first particle if all the following particles are
        // descendant s of the first particle.
        ...
        if(ccSigSeqDcyBrs == sigSeqDcyBrs)
          m_vICcSigSeqDcyBrs.push_back(0);
        else
          m_vICcSigSeqDcyBrs.push_back(1);
  ...
  { cout << "No signal inclusive sequential decay branches are specified." << endl << endl; } else
  {
    ...
    for(unsigned int i = 0; i < m_vVSigPid12.size(); i++)
    {
      for(unsigned int j = 0; j < m_vVSigPid12[i].size(); j++)
      {
        ...
        if(m_pidTxtPnmMap[m_vVSigPid12[i][j]].size() < 7)
          cout << "\t\t\t";
        else if(m_pidTxtPnmMap[m_vVSigPid12[i][j]].size() < 15)
          cout << "\t\t";
        else
          cout << "\t";
        cout << setiosflags(ios::right) << setw(3) << m_vVSigMidx12[i][j]
             << resetiosflags(ios::adjustfield) << endl;
      }
      cout << endl;
    }
    ...
    m_vVSigIncSeqDcyBrsMidxOfHead.clear();
    m_vVCcSigIncSeqDcyBrsMidxOfHead.clear();
    ...
    m_vISigIncSeqDcyBrs.clear();
    m_vICcSigIncSeqDcyBrs.clear();
    m_vNSigIncSeqDcyBrs.clear();
    m_vNCcSigIncSeqDcyBrs.clear();
    ...
    for(unsigned int i = 0; i < m_vVSigPid12.size(); i++)
    {
      ...
      getDcyTr(m_vVSigPid12[i], m_vVSigMidx12[i], sigIncSeqDcyBrs, vSigIncSeqDcyBrsIdxOfHead,
               vSigIncSeqDcyBrsMidxOfHead, vIIncSigIncSeqDcyBrs);
      // remove the branch from e+ and e- to the first particle if all the following particles are
      // descendant s of the first particle.
      ...
      for(unsigned int j = 0; j < m_vSigIncSeqDcyBrs.size(); j++)
      {
        if(sigIncSeqDcyBrs == m_vSigIncSeqDcyBrs[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal inclusive sequential decay branches is same as the " << j + 1
               << ordNumSufj << " signal inclusive sequential decay branches!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
        else if(m_ccSwitch == true && m_vICcSigIncSeqDcyBrs[j] != 0 &&
                sigIncSeqDcyBrs == m_vCcSigIncSeqDcyBrs[j])
        {
          if(i == 0)
            ordNumSufi = "st";
          else if(i == 1)
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal inclusive sequential decay branches is same as the charge conjugate "
                  "counterpart of "
               << j + 1 << ordNumSufj << " signal inclusive sequential decay branches!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      }
      ...
      if(m_ccSwitch == true)
      {
        ...
        vCcSigMidx12.clear();
        for(unsigned int j = 0; j < m_vVSigPid12[i].size(); j++)
          vCcSigPid12.push_back(getCcPid(m_vVSigPid12[i][j]));
        ...
        getDcyTr(vCcSigPid12, vCcSigMidx12, ccSigIncSeqDcyBrs, vCcSigIncSeqDcyBrsIdxOfHead,
                 vCcSigIncSeqDcyBrsMidxOfHead);
        // remove the branch from e+ and e- to the first particle if all the following particles are
        // descendant s of the first particle.
        ...
        if(ccSigIncSeqDcyBrs == sigIncSeqDcyBrs)
          m_vICcSigIncSeqDcyBrs.push_back(0);
        else
          m_vICcSigIncSeqDcyBrs.push_back(1);
  ...
  {
    cout << "No signal intermediate-resonance-allowed decay branches are specified." << endl
         << endl;
  }
  else
  {
    ...
    m_vSigIRADcyBr.clear();
    m_vCcSigIRADcyBr.clear();
    list<int> sigIRADcyBr, ccSigIRADcyBr;
    m_vISigIRADcyBr.clear();
    m_vICcSigIRADcyBr.clear();
    m_vNSigIRADcyBr.clear();
    m_vNCcSigIRADcyBr.clear();
    for(unsigned int i = 0; i < m_vVSigPid3.size(); i++)
    {
      sigIRADcyBr.clear();
      for(unsigned int j = 1; j < m_vVSigPid3[i].size(); j++)
        sigIRADcyBr.push_back(m_vVSigPid3[i][j]);
      ...
      for(unsigned int j = 0; j < m_vSigIRADcyBr.size(); j++)
      {
        if(sigIRADcyBr == m_vSigIRADcyBr[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal intermediate-resonance-allowed decay branch is same as the " << j + 1
               << ordNumSufj << " signal intermediate-resonance-allowed decay branch!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
        else if(m_ccSwitch == true && m_vICcSigIRADcyBr[j] != 0 &&
                sigIRADcyBr == m_vCcSigIRADcyBr[j])
        {
          if(i == 0)
            ordNumSufi = "st";
          else if(i == 1)
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal intermediate-resonance-allowed decay branch is same as the charge "
                  "conjugate counterpart of "
               << j + 1 << ordNumSufj << " signal intermediate-resonance-allowed decay branch!"
               << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      ...
      if(m_ccSwitch == true)
      {
        ...
        if(ccSigIRADcyBr == sigIRADcyBr)
          m_vICcSigIRADcyBr.push_back(0);
        else
          m_vICcSigIRADcyBr.push_back(1);
  ...
  {
    cout << "No signal inclusive or intermediate-resonance-allowed sequential decay branches are "
            "specified."
         << endl
         << endl;
  }
  ...
  {
    cout << "Signal inclusive or intermediate-resonance-allowed sequential decay branches:" << endl
         << endl;
    for(unsigned int i = 0; i < m_vVSigPid123.size(); i++)
    {
      for(unsigned int j = 0; j < m_vVSigPid123[i].size(); j++)
      {
        ...
        if(m_pidTxtPnmMap[m_vVSigPid123[i][j]].size() < 7)
          cout << "\t\t\t";
        else if(m_pidTxtPnmMap[m_vVSigPid123[i][j]].size() < 15)
          cout << "\t\t";
        else
          cout << "\t";
        cout << setiosflags(ios::right) << setw(3) << m_vVSigMidx123[i][j]
             << resetiosflags(ios::adjustfield) << endl;
      }
      cout << endl;
    }
    ...
    m_vVSigIncOrIRASeqDcyBrsMidxOfHead.clear();
    m_vVCcSigIncOrIRASeqDcyBrsMidxOfHead.clear();
    ...
    m_vISigIncOrIRASeqDcyBrs.clear();
    m_vICcSigIncOrIRASeqDcyBrs.clear();
    m_vNSigIncOrIRASeqDcyBrs.clear();
    m_vNCcSigIncOrIRASeqDcyBrs.clear();
    ...
    for(unsigned int i = 0; i < m_vVSigPid123.size(); i++)
    {
      ...
      getDcyTr(m_vVSigPid123[i], m_vVSigMidx123[i], sigIncOrIRASeqDcyBrs,
               vSigIncOrIRASeqDcyBrsIdxOfHead, vSigIncOrIRASeqDcyBrsMidxOfHead,
               vIIncSigIncOrIRASeqDcyBrs, vIIRASigIncOrIRASeqDcyBrs);
      // remove the branch from e+ and e- to the first particle if all the following particles are
      // descendant s of the first particle.
      ...
      for(unsigned int j = 0; j < m_vSigIncOrIRASeqDcyBrs.size(); j++)
      {
        ...
        else if(m_ccSwitch == true && m_vICcSigIncOrIRASeqDcyBrs[j] != 0 &&
                sigIncOrIRASeqDcyBrs == m_vCcSigIncOrIRASeqDcyBrs[j])
        {
          if(i == 0)
            ordNumSufi = "st";
          else if(i == 1)
          ...
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal inclusive or intermediate-resonance-allowed sequential decay branches "
                  "is same as the charge conjugate counterpart of "
               << j + 1 << ordNumSufj
               << " signal inclusive or intermediate-resonance-allowed sequential decay branches!"
               << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
      ...
      if(m_ccSwitch == true)
      {
        ...
        vCcSigMidx123.clear();
        for(unsigned int j = 0; j < m_vVSigPid123[i].size(); j++)
          vCcSigPid123.push_back(getCcPid(m_vVSigPid123[i][j]));
        ...
        getDcyTr(vCcSigPid123, vCcSigMidx123, ccSigIncOrIRASeqDcyBrs,
                 vCcSigIncOrIRASeqDcyBrsIdxOfHead, vCcSigIncOrIRASeqDcyBrsMidxOfHead);
        // remove the branch from e+ and e- to the first particle if all the following particles are
        // descendant s of the first particle.
        ...
        if(ccSigIncOrIRASeqDcyBrs == sigIncOrIRASeqDcyBrs)
          m_vICcSigIncOrIRASeqDcyBrs.push_back(0);
        else
          m_vICcSigIncOrIRASeqDcyBrs.push_back(1);
    ...
    for(unsigned int i = 0; i < m_vSigIncOrIRASeqDcyBrs.size(); i++)
    {
      ...
      for(unsigned int j = 0; j < sigIncOrIRASeqDcyBrs.size(); j++)
      {
        ...
        {
          cout << endl << endl;
          cerr << "Error: The Branch is not only inclusive but also . Such Branches are not "
                  "supported in the current version of the \"TopoAna\" program!"
               << endl;
          cerr << "Infor: If you really need the function, Please contact me (zhouxy@buaa.edu.cn) "
                  "and then I will try to implement the function for you."
               << endl;
          exit(-1);
        }
        cout << endl;
  ...
  if(m_vVSigPid4.size() == 0) { cout << "No signal decay trees are specified." << endl << endl; }
  else
  {
    ...
    for(unsigned int i = 0; i < m_vVSigPid4.size(); i++)
    {
      for(unsigned int j = 0; j < m_vVSigPid4[i].size(); j++)
      {
        ...
        if(m_pidTxtPnmMap[m_vVSigPid4[i][j]].size() < 7)
          cout << "\t\t\t";
        else if(m_pidTxtPnmMap[m_vVSigPid4[i][j]].size() < 15)
          cout << "\t\t";
        else
          cout << "\t";
        cout << setiosflags(ios::right) << setw(3) << m_vVSigMidx4[i][j]
             << resetiosflags(ios::adjustfield) << endl;
      }
      cout << endl;
    }
    ...
    m_vNSigDcyTr.clear();
    m_vNCcSigDcyTr.clear();
    m_vSigDcyFSt.clear();
    m_vCcSigDcyFSt.clear();
    list<int> sigDcyFSt, ccSigDcyFSt;
    m_vISigDcyFSt.clear();
    m_vICcSigDcyFSt.clear();
    m_vNSigDcyFSt.clear();
    m_vNCcSigDcyFSt.clear();
    m_iSigDcyTrISigDcyFStMap.clear();
    m_iSigDcyTrICcSigDcyFStMap.clear();
    ...
    for(unsigned int i = 0; i < m_vVSigPid4.size(); i++)
    {
      ...
      for(unsigned int j = 0; j < m_vSigDcyTr.size(); j++)
      {
        if(sigDcyTr == m_vSigDcyTr[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi << " signal decay tree is same as the "
               << j + 1 << ordNumSufj << " signal decay tree!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
        else if(m_ccSwitch == true && m_vICcSigDcyTr[j] != 0 && sigDcyTr == m_vCcSigDcyTr[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal decay tree is same as the charge conjugate counterpart of " << j + 1
               << ordNumSufj << " signal decay tree!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      }
      ...
      if(m_ccSwitch == true)
      {
        ...
        vCcSigMidx.clear();
        for(unsigned int j = 0; j < m_vVSigPid4[i].size(); j++)
          vCcSigPid.push_back(getCcPid(m_vVSigPid4[i][j]));
        ...
        if(ccSigDcyTr == sigDcyTr)
          m_vICcSigDcyTr.push_back(0);
        else
          m_vICcSigDcyTr.push_back(1);
        m_vNCcSigDcyTr.push_back(0);
      }

      iSigDcyFSt = -1; // If the variable is still equal to -1 after the following loop, then the
                       // signal decay final states is a new signal decay final states.
      for(unsigned int j = 0; j < m_vSigDcyFSt.size(); j++)
      {
        if(sigDcyFSt == m_vSigDcyFSt[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Infor: The final state of the " << i + 1 << ordNumSufi
               << " signal decay tree is same as that of the " << j + 1 << ordNumSufj
               << " signal decay tree." << endl
               << endl;
          iSigDcyFSt = j;
        ...
        else if(m_ccSwitch == true && m_vICcSigDcyFSt[j] != 0 && sigDcyFSt == m_vCcSigDcyFSt[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Infor: The final state of the " << i + 1 << ordNumSufi
               << " signal decay tree is same as that of the charge conjugate counterpart of "
               << j + 1 << ordNumSufj << " signal decay tree." << endl
               << endl;
          iSigDcyFSt   = j;
          iCcSigDcyFSt = -1;
  ...
  { cout << "No signal decay final states are specified." << endl << endl; } else
  {
    ...
    m_vSigDcyFSt2.clear();
    m_vCcSigDcyFSt2.clear();
    list<int> sigDcyFSt2, ccSigDcyFSt2;
    m_vISigDcyFSt2.clear();
    m_vICcSigDcyFSt2.clear();
    m_vNSigDcyFSt2.clear();
    m_vNCcSigDcyFSt2.clear();
    for(unsigned int i = 0; i < m_vVSigPid5.size(); i++)
    {
      sigDcyFSt2.clear();
      for(unsigned int j = 0; j < m_vVSigPid5[i].size(); j++)
        sigDcyFSt2.push_back(m_vVSigPid5[i][j]);
      ...
      for(unsigned int j = 0; j < m_vSigDcyFSt2.size(); j++)
      {
        if(sigDcyFSt2 == m_vSigDcyFSt2[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal decay final state is same as the " << j + 1 << ordNumSufj
               << " signal decay final state!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
        else if(m_ccSwitch == true && m_vICcSigDcyFSt2[j] != 0 && sigDcyFSt2 == m_vCcSigDcyFSt2[j])
        {
          ...
          if(j == 0)
            ...
            ordNumSufj = "rd";
          cerr << "Error: The " << i + 1 << ordNumSufi
               << " signal decay final state is same as the charge conjugate counterpart of "
               << j + 1 << ordNumSufj << " signal decay final state!" << endl;
          cerr << "Infor: Please check the input card and remove one of them." << endl;
          exit(-1);
        }
      }
      ...
      if(m_ccSwitch == true)
      {
        ...
        if(ccSigDcyFSt2 == sigDcyFSt2)
          m_vICcSigDcyFSt2.push_back(0);
        else
          m_vICcSigDcyFSt2.push_back(1);
  ...
  if((m_sttaDcyTrsAndDcyFSts == false) || (m_vPttaPid.size() != 0) || (m_vVPttaPid1.size() != 0) ||
     (m_vSigPid.size() != 0) || (m_vVSigPid1.size() != 0) || (m_vVSigPid2.size() != 0) ||
     (m_vVSigPid12.size() != 0) || (m_vVSigPid3.size() != 0) || (m_vVSigPid123.size() != 0) ||
     (m_vVSigPid4.size() != 0) || (m_vVSigPid5.size() != 0))
    haveTopoAnaTasks = true;
