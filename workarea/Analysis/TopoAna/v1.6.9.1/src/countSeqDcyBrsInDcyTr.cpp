...
unsigned int topoana::countSeqDcyBrsInDcyTr(vector< list<int> > & seqDcyBrs, vector<int> vIdxOfHead1, vector<int> vMidxOfHead1, vector< list<int> > & dcyTr, vector<int> vIdxOfHead2, vector<int> vMidxOfHead2)
{
  unsigned int nCount=0;
  if(seqDcyBrs.size()==0)
    {
      cerr<<"Infor: The size of the vector for the sequential decay branches is zero!"<<endl<<endl;
      return nCount;
    }
  if(dcyTr.size()==0)
    {
  ...
  if(seqDcyBrs.size()<=dcyTr.size())
    {
      /*for(unsigned int i=0;i<seqDcyBrs.size();i++)
        {
          list<int>::iterator liit,liitTmp=seqDcyBrs[i].end();liitTmp--;
          for(liit=seqDcyBrs[i].begin();liit!=seqDcyBrs[i].end();liit++)
            {
      ...
      for(unsigned int i=1;i<seqDcyBrs.size();i++)
        {
      ...
      for(unsigned int i=0;i<vVIDcyBr2WRTIDcyBr1.size();i++)
        {
          for(unsigned int j=1;j<seqDcyBrs.size();j++)
            {
              for(unsigned int k=vVIDcyBr2WRTIDcyBr1[i][j-1]+1;k<dcyTr.size();k++)
                {
                  if(dcyTr[k]==seqDcyBrs[j]&&(vMidxOfHead2[k]==vIdxOfHead2[k]||vMidxOfHead2[k]==vIdxOfHead2[(unsigned int) vVIDcyBr2WRTIDcyBr1[i][(unsigned int) vMIDcyBr1WRTIDcyBr1[j]]]))
                    {
          ...
          if(vVIDcyBr2WRTIDcyBr1[i].size()==seqDcyBrs.size()) nCount++;
        }
    }
