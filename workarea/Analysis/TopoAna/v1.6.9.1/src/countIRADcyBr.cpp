...
// The fourth argument is appended for the special usage in the "countIncOrIRASeqDcyBrsInDcyTr"
// method.
unsigned int topoana::countIRADcyBr(vector<int>& vPid, vector<int>& vMidx, list<int> IRADcyBr,
                                    bool areHeadsRequiredToBeMatched)
{
  vector<int> vIdx;
  vIdx.clear();
  ...
  for(unsigned int i = 0; i < vIdx.size(); i++)
  {
    ...
    for(unsigned int j = vIdx[i] + 1; j < vPid.size(); j++)
    {
      ...
      if((specifiedP == true || fsp == true) && (descendantOfOneSpecifiedP == false))
      {
        ...
        // The following condition "abs(vPid[j])==22" is exerted specially for the comparison
        // without final state photons.
        if(fromTheP == true)
        {
          // If the gFSR photons are ignored, then they don't show themselves up here. If they are
          // not ignored, then their PDG codes are modified to be -22 or m_pidOfGFSRGam in the
          // "sortPs" function.
