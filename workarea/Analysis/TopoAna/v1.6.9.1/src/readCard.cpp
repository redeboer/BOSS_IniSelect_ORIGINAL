...
void topoana::readCard(string topoAnaCardFlNm)
{
  ...
  if(!fin)
  {
    ...
    if(topoAnaCardFlNm == "topoana.card")
    {
      ...
      cerr << "Infor: Do you forget to append the name of your own card file to the end of the "
              "executable?"
           << endl;
      cerr << "Infor: If yes, please specify it along with the executable by the command line "
              "\"(pathOfTheExecutable/)topoana.exe nameOfYourOwnCardFile\""
           << endl;
    }
    else
    {
  ...
  while(!fin.eof())
  {
    ...
    else if(line == "% Names of input root files")
    {
      ...
      read1stLineOrCloseCurly(
        fin, line, true,
        "No root file names are found in the item with the prompt \"% Names of input root files");
    ...
    else if(line == "% Tree name")
    {
      ...
      read1stLineOrCloseCurly(fin, line, true,
                              "No tree name is found in the item with the prompt \"% Tree name");
      m_trNm = line;
    ...
    else if(line == "% Branch name of the number of particles")
    {
      ...
      read1stLineOrCloseCurly(fin, line, true,
                              "No branch name of the number of particles is found in the item with "
                              "the prompt \"% Branch name of the number of particles");
      m_brNmOfNps = line;
    ...
    else if(line == "% Branch name of the array of particle identifications")
    {
      ...
      read1stLineOrCloseCurly(
        fin, line, true,
        "No branch name of the array of particle identifications is found in the item with the "
        "prompt \"% Branch name of the array of particle identifications");
      m_brNmOfPid = line;
    ...
    else if(line == "% Branch name of the array of the mother indeces of particles")
    {
      ...
      read1stLineOrCloseCurly(
        fin, line, true,
        "No branch name of the array of the mother indeces of particles is found in the item with "
        "the prompt \"% Branch name of the array of the mother indeces of particles");
      m_brNmOfMidx = line;
    ...
    else if(line == "% Main name of output files")
    {
      ...
      read1stLineOrCloseCurly(
        fin, line, true,
        "No main file name is found in the item with the prompt \"% Main name of output files");
    ...
    else if(line == "% Method to apply cut to array variables (Two options: T and F. Default: F)")
    {
      ...
      if(line != "}")
      {
        if(line == "T") { m_cutMethod = true; }
        else if(line != "F")
        {
          cerr << "Error: The input \"" << line
               << "\" for the item \"Method to apply cut to array variables\" is invalid!" << endl;
          cerr << "Infor: It should be \"T\" or \"F\"." << endl;
          cerr << "Infor: Please check it." << endl;
          exit(-1);
        }
        readCloseCurly(
          fin, line, "% Method to apply cut to array variables (Two options: T and F. Default: F)");
      }
    }
    else if(line ==
            "% Process charge conjugate objects together (Two options: Y and N. Default: N)")
    {
      ...
      if(line != "}")
      {
        if(line == "Y") { m_ccSwitch = true; }
        else if(line != "N")
        {
          cerr << "Error: The input \"" << line
               << "\" for the item \"Process charge conjugate objects together\" is invalid!"
               << endl;
    ...
    else if(line == "% Ignore gISR photons (Two options: Y and N. Default: Y)")
    {
      ...
      read1stLineOrCloseCurly(fin, line, false,
                              "% Ignore gISR photons (Two options: Y and N. Default: Y)");
      if(line != "}")
      {
        if(line == "N") { m_ignoreGISR = false; }
        else if(line != "Y")
        {
          cerr << "Error: The input \"" << line
               << "\" for the item \"Ignore gISR photons\" is invalid!" << endl;
          cerr << "Infor: It should be \"Y\" or \"N\"." << endl;
          cerr << "Infor: Please check it." << endl;
          exit(-1);
        }
    ...
    else if(line == "% Ignore gFSR photons (Two options: Y and N. Default: Y)")
    {
      ...
      if(line != "}")
      {
        if(line == "N") { m_ignoreGFSR = false; }
        else if(line != "Y")
        {
          cerr << "Error: The input \"" << line
               << "\" for the item \"Ignore gFSR photons\" is invalid!" << endl;
          cerr << "Infor: It should be \"Y\" or \"N\"." << endl;
          cerr << "Infor: Please check it." << endl;
          exit(-1);
        }
    ...
    else if(line == "% Maximum hierarchy of heading decay branches to be processed in each event")
    {
      ...
      if(line != "}")
      {
        ...
        iss >> m_hHdDcyBrsMax;
        readCloseCurly(
          fin, line, "% Maximum hierarchy of heading decay branches to be processed in each event");
      }
    }
    ...
    else if(line == "% Ignore the decay of the daughters of the following particles")
    {
      ...
      if(line != "}")
      {
        ...
        while(1)
        {
          readExtraLinesOrCloseCurly(
            fin, line, "% Ignore the decay of the daughters of the following particles");
          if(line == "}") { break; }
          else
          {
    ...
    else if(line == "% Suppress the first branches of decay trees in the output txt/tex/pdf files "
                    "(Two options: Y and N. Default: N)")
    {
      readOpenCurly(fin, line,
                    "% Suppress the first branches of decay trees in the output txt/tex/pdf files "
                    "(Two options: Y and N. Default: N)");
      read1stLineOrCloseCurly(fin, line, false,
                              "% Suppress the first branches of decay trees in the output "
                              "txt/tex/pdf files (Two options: Y and N. Default: N)");
      if(line != "}")
      {
        if(line == "Y") { m_spr1stBrsOfDcyTrs = true; }
        else if(line != "N")
        {
          cerr << "Error: The input \"" << line
               << "\" for the item \"Suppress the first branches of decay trees in the output "
                  "txt/tex/pdf files\" is invalid!"
               << endl;
          cerr << "Infor: It should be \"Y\" or \"N\"." << endl;
          cerr << "Infor: Please check it." << endl;
          exit(-1);
        }
        readCloseCurly(fin, line,
                       "% Suppress the first branches of decay trees in the output txt/tex/pdf "
                       "files (Two options: Y and N. Default: N)");
      }
    }
    else if(line == "% Suppress the topology tags in the output tex/pdf files (Two options: Y and "
                    "N. Default: N)")
    {
      readOpenCurly(fin, line,
                    "% Suppress the topology tags in the output tex/pdf files (Two options: Y and "
                    "N. Default: N)");
      read1stLineOrCloseCurly(fin, line, false,
                              "% Suppress the topology tags in the output tex/pdf files (Two "
                              "options: Y and N. Default: N)");
      if(line != "}")
      {
        if(line == "Y") { m_sprTopoTags = true; }
        else if(line != "N")
        {
          cerr << "Error: The input \"" << line
               << "\" for the item \"Suppress the topology tags in the output tex/pdf files\" is "
                  "invalid!"
               << endl;
          cerr << "Infor: It should be \"Y\" or \"N\"." << endl;
          cerr << "Infor: Please check it." << endl;
        ...
        readCloseCurly(fin, line,
                       "% Suppress the topology tags in the output tex/pdf files (Two options: Y "
                       "and N. Default: N)");
      }
    }
    else if(line == "% Maximum number of entries to be saved in a single output root file")
    {
      ...
      read1stLineOrCloseCurly(
        fin, line, false, "% Maximum number of entries to be saved in a single output root file");
      if(line != "}")
      {
        istringstream iss;
        iss.clear();
    ...
    else if(line == "% Skip the topology analysis of decay trees and decay final states (Two "
                    "options: Y and N. Default: N)")
    {
      readOpenCurly(fin, line,
                    "% Skip the topology analysis of decay trees and decay final states (Two "
                    "options: Y and N. Default: N)");
      read1stLineOrCloseCurly(fin, line, false,
                              "% Skip the topology analysis of decay trees and decay final states "
                              "(Two options: Y and N. Default: N)");
      if(line != "}")
      {
        if(line == "Y") { m_sttaDcyTrsAndDcyFSts = true; }
        else if(line != "N")
        {
          cerr << "Error: The input \"" << line
               << "\" for the item \"Skip the topology analysis of decay trees and decay final "
                  "states\" is invalid!"
               << endl;
          cerr << "Infor: It should be \"Y\" or \"N\"." << endl;
          cerr << "Infor: Please check it." << endl;
    ...
    else if(line == "% Perform the topology analysis of the decay branches begun with the "
                    "following particles")
    {
      readOpenCurly(
        fin, line,
      ...
      if(line != "}")
      {
        ...
        if(!iss.eof())
          ...
          nDcyBrsMax = ULONG_MAX;
        m_vNDcyBrsMax.push_back(nDcyBrsMax);
        while(1)
        {
          readExtraLinesOrCloseCurly(fin, line,
                                     "% Perform the topology analysis of the decay branches begun "
    ...
    else if(line == "% Perform the topology analysis of the exclusive decay branches matched with "
                    "the following inclusive decay branches")
    {
      readOpenCurly(fin, line,
                    "% Perform the topology analysis of the exclusive decay branches matched with "
                    "the following inclusive decay branches");
      read1stLineOrCloseCurly(fin, line, false,
                              "% Perform the topology analysis of the exclusive decay branches "
                              "matched with the following inclusive decay branches");
      if(line != "}")
      {
        istringstream iss;
        iss.clear();
    ...
    else if(line == "% Signal inclusive sequential decay branches")
    {
      ...
      if(line != "}")
      {
        ...
        vSigPid12.clear();
        // The asterisk symbol "*" is used as a short version of the word "anything" in order to
        ...
        // Pay attention to that if sigTxtPnm12==asterisk, then 1 is returned. Please do not get
        // this wrong.
            ...
            "% Signal inclusive or intermediate-resonance-allowed sequential decay branches")
    {
      ...
      if(line != "}")
      {
        ...
        // Pay attention to that if sigTxtPnm123==asterisk, then 1 is returned. Please do not get
        // this wrong.
        ...
        while(1)
        {
          ...
          else
          {
            ...
            // Pay attention to that if sigTxtPnm123==asterisk, then 1 is returned. Please do not
            // get this wrong.
            if(sigTxtPnm123 == asterisk) sigTxtPnm123 = "anything";
            sigPid123 = getPidFromTxtPnm(sigTxtPnm123);
            vSigPid123.push_back(sigPid123);
