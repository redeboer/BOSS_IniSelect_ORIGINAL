...
void topoana::writeRsltIntoTexFl()
{
  ...
  fout << "\\title{Topology Analysis \\footnote{\\normalsize{This package is implemented with "
          "reference to a program called {\\sc Topo}, which is developed by Prof. Shuxian Du from "
          "Zhengzhou University in China and has been widely used by people in BESIII "
          "collaboration. Several years ago, when I was a PhD student working on BESIII "
          "experiment, I learned the idea of topology analysis and a lot of programming techniques "
          "from the {\\sc Topo} program. So, I really appreciate Prof. Du's original work very "
          "much. To meet my own needs and to practice developing analysis tools with C++, ROOT and "
          "LaTex, I wrote the package from scratch. At that time, the package functioned well but "
          "was relatively simple. At the end of last year (2017), my co-supervisor, Prof. "
          "Chengping Shen reminded me that it could be a useful tool for Belle II experiment as "
          "well. So, I revised and extended it, making it more well-rounded and suitable for Belle "
          "II experiment. Here, I would like to thank Prof. Du for his orignial work, Prof. Shen "
          "for his suggestion and encouragement, and Xian Xiong, Wencheng Yan, Sen Jia, Yubo Li, "
          "Suxian Li, Longke Li, Guanda Gong, Junhao Yin, Xiaoping Qin, Xiqing Hao, HongPeng Wang, "
          "JiaWei Zhang, Yeqi Chen and Runqiu Ma for their efforts in helping me test the program. "
          "I would also like to thank my good friend Xi Chen, a professional programmer, for his "
          "suggestion in core algrithm and efficiency.}} \\\\ \\vspace{0.1cm} \\Large{("
       << m_verNum << ")}}" << endl;
  fout << "\\author{Xingyu Zhou \\footnote{\\normalsize{Email: zhouxy@buaa.edu.cn}} \\\\ "
          "\\vspace{0.1cm} Beihang University}"
       << endl;
  fout << "\\maketitle" << endl;
  ...
  unsigned int       nBrsInALine =
    6; // The value of this variable is changed to 5 for the case of the table titled signal decay
       // trees and their respective final states.
  unsigned int  nLines;
  ...
  if(m_sttaDcyTrsAndDcyFSts == false)
  {
    ...
    for(unsigned long i = 0; i < nDcyTrsToBePrtd; i++)
    {
      ...
      for(unsigned int j = 0; j < dcyTr.size(); j++)
      {
        ...
        dcyBr.clear();
        dcyBr = dcyTr[j];
        // Since a lot of pi0s are produced and almost all of them decay to gammma pairs, to save
        // paper and for convenience of readers, all branches of pi0 to gamma pairs are not
        // outputted into the tex file, and hence not printed in the pdf file.
        ...
        liit2++;
        liit2++;
        if(((*liit) == 111) && (dcyBr.size() == 3) && ((*liit1) == 22) && ((*liit2) == 22))
          continue;
        nVldDcyBrs++;
        ...
        // The following bool variable is used to judge whether the current branch is the last one
        // outputed or not.
        ...
        for(unsigned int k = j + 1; k < dcyTr.size(); k++)
        {
          ...
          liitTmp2++;
          liitTmp2++;
          if(((*liitTmp) != 111) || (dcyBrTmp.size() != 3) || ((*liitTmp1) != 22) ||
             ((*liitTmp2) != 22))
          {
      ...
      if(m_sprTopoTags == false)
        fout << m_vIDcyTr[i] << " & " << m_iDcyTrIDcyFStMap[m_vIDcyTr[i]] << " & ";
      fout << m_vNDcyTr[i] << " & ";
      if(m_ccSwitch == true)
      {
        if(m_vICcDcyTr[i] == 0)
        ...
        if(i == nDcyTrsToBePrtd - 1 && nDcyTrsToBePrtd < m_vDcyTr.size())
        {
          ...
          if(m_spr1stBrsOfDcyTrs == true)
          {
            ...
            for(unsigned long i = nDcyTrsToBePrtd; i < m_vDcyTr.size(); i++)
            {
              ...
              {
                if(m_vDcyTr[i][0].size() != 3) { isEpEmToAPInAllOtherDcyTrs = false; }
                else
                {
          ...
          if(m_ccSwitch == true)
          {
            fout << "---"
                 ...
                 << " & ";
            for(unsigned long i = nDcyTrsToBePrtd; i < m_vDcyTr.size(); i++)
              nCmltEtrsOfRest = nCmltEtrsOfRest + m_vNDcyTr[i] + m_vNCcDcyTr[i];
          }
          else
          {
            for(unsigned long i = nDcyTrsToBePrtd; i < m_vDcyTr.size(); i++)
              nCmltEtrsOfRest = nCmltEtrsOfRest + m_vNDcyTr[i];
          }
    ...
    nCmltEtrs = 0;
    unsigned long nDcyFStsToBePrtd =
      m_vDcyFSt.size() < m_nDcyFStsToBePrtdMax ? m_vDcyFSt.size() : m_nDcyFStsToBePrtdMax;
        ...
        if(i == nDcyFStsToBePrtd - 1 && nDcyFStsToBePrtd < m_vDcyFSt.size())
        {
          ...
          if(m_ccSwitch == true)
          {
            fout << "---"
                 ...
                 << "---"
                 << " & ";
            for(unsigned long i = nDcyFStsToBePrtd; i < m_vDcyFSt.size(); i++)
              nCmltEtrsOfRest = nCmltEtrsOfRest + m_vNDcyFSt[i] + m_vNCcDcyFSt[i];
          }
          else
          {
            for(unsigned long i = nDcyFStsToBePrtd; i < m_vDcyFSt.size(); i++)
              nCmltEtrsOfRest = nCmltEtrsOfRest + m_vNDcyFSt[i];
          }
          nCmltEtrs = nCmltEtrs + nCmltEtrsOfRest;
          fout << nCmltEtrsOfRest << " & " << nCmltEtrs << " \\\\" << endl;
          fout << "\\hline" << endl;
  ...
  if(m_vVDcyBrP.size() > 0)
  {
    for(unsigned int i = 0; i < m_vVDcyBrP.size(); i++)
    {
      ...
      unsigned long nDcyBrPToBePrtd =
        m_vVDcyBrP[i].size() < m_vNDcyBrsMax[i] ? m_vVDcyBrP[i].size() : m_vNDcyBrsMax[i];
      for(unsigned int j = 0; j < nDcyBrPToBePrtd; j++)
      {
        ...
        if(m_ccSwitch == true)
        {
          if(m_vPttaICcPid[i] == 0 && m_vVIDcyBrCcP[i][j] == 0)
            fout << "---";
          else
            fout << m_vVNDcyBrCcP[i][j];
          ...
          if(j == nDcyBrPToBePrtd - 1 && nDcyBrPToBePrtd < m_vVDcyBrP[i].size())
          {
            ...
            if(m_ccSwitch == true)
            {
              ...
              else
              {
                ...
                for(unsigned int j = nDcyBrPToBePrtd; j < m_vVDcyBrP[i].size(); j++)
                  nCmltCasesOfRestTemp1 = nCmltCasesOfRestTemp1 + m_vVNDcyBrP[i][j];
                for(unsigned int j = nDcyBrPToBePrtd; j < m_vVDcyBrP[i].size(); j++)
                  nCmltCasesOfRestTemp2 = nCmltCasesOfRestTemp2 + m_vVNDcyBrCcP[i][j];
                fout << nCmltCasesOfRestTemp1 << " & " << nCmltCasesOfRestTemp2 << " & ";
              }
              for(unsigned int j = nDcyBrPToBePrtd; j < m_vVDcyBrP[i].size(); j++)
                nCmltCasesOfRest = nCmltCasesOfRest + m_vVNDcyBrP[i][j] + m_vVNDcyBrCcP[i][j];
            }
            else
            {
              for(unsigned int j = nDcyBrPToBePrtd; j < m_vVDcyBrP[i].size(); j++)
                nCmltCasesOfRest = nCmltCasesOfRest + m_vVNDcyBrP[i][j];
            }
  ...
  if(m_vVDcyBrIncDcyBr.size() > 0)
  {
    for(unsigned int i = 0; i < m_vVDcyBrIncDcyBr.size(); i++)
    {
      ...
      unsigned long nCmltCases             = 0;
      unsigned long nDcyBrIncDcyBrToBePrtd = m_vVDcyBrIncDcyBr[i].size() < m_vNDcyBrsMax1[i] ?
                                               m_vVDcyBrIncDcyBr[i].size() :
                                               m_vNDcyBrsMax1[i];
      for(unsigned int j = 0; j < nDcyBrIncDcyBrToBePrtd; j++)
      {
        ...
        if(m_ccSwitch == true)
        {
          if(m_vPttaICcPid[i] == 0 && m_vVIDcyBrCcIncDcyBr[i][j] == 0)
            fout << "---";
          else
            fout << m_vVNDcyBrCcIncDcyBr[i][j];
        ...
        if(j % nLinesMax == nLinesMax - 1 || j == nDcyBrIncDcyBrToBePrtd - 1)
        {
          if(j == nDcyBrIncDcyBrToBePrtd - 1 &&
             nDcyBrIncDcyBrToBePrtd < m_vVDcyBrIncDcyBr[i].size())
          {
            if(j % nLinesMax == nLinesMax - 1)
            {
              fout << "\\end{tabular}" << endl;
            ...
            if(m_ccSwitch == true)
            {
              ...
              else
              {
                unsigned long nCmltCasesOfRestTemp1 = 0;
                unsigned long nCmltCasesOfRestTemp2 = 0;
                for(unsigned int j = nDcyBrIncDcyBrToBePrtd; j < m_vVDcyBrIncDcyBr[i].size(); j++)
                  nCmltCasesOfRestTemp1 = nCmltCasesOfRestTemp1 + m_vVNDcyBrIncDcyBr[i][j];
                for(unsigned int j = nDcyBrIncDcyBrToBePrtd; j < m_vVDcyBrIncDcyBr[i].size(); j++)
                  nCmltCasesOfRestTemp2 = nCmltCasesOfRestTemp2 + m_vVNDcyBrCcIncDcyBr[i][j];
                fout << nCmltCasesOfRestTemp1 << " & " << nCmltCasesOfRestTemp2 << " & ";
              }
              for(unsigned int j = nDcyBrIncDcyBrToBePrtd; j < m_vVDcyBrIncDcyBr[i].size(); j++)
                nCmltCasesOfRest =
                  nCmltCasesOfRest + m_vVNDcyBrIncDcyBr[i][j] + m_vVNDcyBrCcIncDcyBr[i][j];
            }
            else
            {
              for(unsigned int j = nDcyBrIncDcyBrToBePrtd; j < m_vVDcyBrIncDcyBr[i].size(); j++)
                nCmltCasesOfRest = nCmltCasesOfRest + m_vVNDcyBrIncDcyBr[i][j];
            }
            nCmltCases = nCmltCases + nCmltCasesOfRest;
            fout << nCmltCasesOfRest << " & " << nCmltCases << " \\\\" << endl;
            fout << "\\hline" << endl;
  ...
  if(m_vSigIncDcyBr.size() > 0)
  {
    ...
    for(unsigned int i = 0; i < m_vSigIncDcyBr.size(); i++)
    {
      ...
      if(m_ccSwitch == true)
      {
        if(m_vICcSigIncDcyBr[i] > 0)
          fout << m_vNCcSigIncDcyBr[i];
        else
          fout << "---";
  ...
  if(m_vSigSeqDcyBrs.size() > 0)
  {
    ...
    for(unsigned int i = 0; i < m_vSigSeqDcyBrs.size(); i++)
    {
      ...
      if(m_ccSwitch == true)
      {
        if(m_vICcSigSeqDcyBrs[i] > 0)
          fout << m_vNCcSigSeqDcyBrs[i];
        else
          fout << "---";
  ...
  if(m_vSigIncSeqDcyBrs.size() > 0)
  {
    ...
    for(unsigned int i = 0; i < m_vSigIncSeqDcyBrs.size(); i++)
    {
      ...
      if(m_ccSwitch == true)
      {
        if(m_vICcSigIncSeqDcyBrs[i] > 0)
          fout << m_vNCcSigIncSeqDcyBrs[i];
        else
          fout << "---";
  ...
  if(m_vSigIRADcyBr.size() > 0)
  {
    ...
    for(unsigned int i = 0; i < m_vSigIRADcyBr.size(); i++)
    {
      if(i % nLinesMax == 0)
      {
        ...
        fout << "\\begin{table}[htbp!]" << endl;
        if(i == 0)
          fout << "\\caption{Signal intermediate-resonance-allowed decay branches.}" << endl;
        fout << "\\small" << endl;
        fout << "\\centering" << endl;
        fout << "\\begin{tabular}{|c|c|c|c|c|";
        if(m_ccSwitch == true) fout << "c|c|";
      ...
      if(m_ccSwitch == true)
      {
        if(m_vICcSigIRADcyBr[i] > 0)
          fout << m_vNCcSigIRADcyBr[i];
        else
          fout << "---";
  ...
  if(m_vSigIncOrIRASeqDcyBrs.size() > 0)
  {
    ...
    for(unsigned int i = 0; i < m_vSigIncOrIRASeqDcyBrs.size(); i++)
    {
      if(nLines == 0)
      {
        ...
        if(i == 0)
        {
          fout << "\\caption{Signal inclusive or intermediate-resonance-allowed sequential decay "
                  "branches.}"
               << endl;
          nLines++;
        }
        ...
        fout << "}" << endl;
        fout << "\\hline" << endl;
        fout << "index & \\thead{signal inclusive or intermediate-resonance-allowed \\\\ "
                "sequential decay branches} & ";
      ...
      for(unsigned int j = 0; j < sigIncOrIRASeqDcyBrs.size(); j++)
      {
        ...
        if(m_vVIIncSigIncOrIRASeqDcyBrs[i][j] == 1)
          writePnmFromPid(fout, "TexPnm", m_pidOfAnything);
        // The following bool variable is used to judge whether the current branch is the last one
        // outputed or not.
      ...
      if(m_ccSwitch == true)
      {
        if(m_vICcSigIncOrIRASeqDcyBrs[i] > 0)
          fout << m_vNCcSigIncOrIRASeqDcyBrs[i];
        else
          fout << "---";
  ...
  if(m_vSigDcyTr.size() > 0)
  {
    ...
    for(unsigned int i = 0; i < m_vSigDcyTr.size(); i++)
    {
      ...
      for(unsigned int j = 0; j < sigDcyTr.size(); j++)
      {
        ...
        liit2++;
        liit2++;
        if(((*liit) == 111) && (sigDcyBr.size() == 3) && ((*liit1) == 22) && ((*liit2) == 22))
          continue;
        nVldSigDcyTr++;
        writePnmFromPid(fout, "TexPnm", (*liit));
        // The condition "j==0" is set for the initial state particle pair e+e-;the condition
        ...
        for(unsigned int k = j + 1; k < sigDcyTr.size(); k++)
        {
          ...
          liitTmp2++;
          liitTmp2++;
          if(((*liitTmp) != 111) || (sigDcyBrTmp.size() != 3) || ((*liitTmp1) != 22) ||
             ((*liitTmp2) != 22))
          {
            lastOneOrAllPi0sToGamPairsBehind = false;
            break;
      ...
      {
        fout << m_vISigDcyTr[i] << " & ";
        if(m_iSigDcyTrICcSigDcyFStMap[m_vISigDcyTr[i]] >= 0)
          fout << m_iSigDcyTrISigDcyFStMap[m_vISigDcyTr[i]];
        else
          fout << "$ " << m_iSigDcyTrISigDcyFStMap[m_vISigDcyTr[i]] << "_{\\rm cc} $";
        fout << " & ";
        if(m_iSigDcyTrIDcyTrMap.find(m_vISigDcyTr[i]) != m_iSigDcyTrIDcyTrMap.end())
        {
          if(m_iSigDcyTrICcDcyTrMap[m_vISigDcyTr[i]] >= 0)
            fout << m_iSigDcyTrIDcyTrMap[m_vISigDcyTr[i]];
          else
            fout << "$ " << m_iSigDcyTrIDcyTrMap[m_vISigDcyTr[i]] << "_{\\rm cc} $";
        }
        else
          fout << "---";
        fout << " & ";
        if(m_iSigDcyFStIDcyFStMap.find(m_iSigDcyTrISigDcyFStMap[m_vISigDcyTr[i]]) !=
           m_iSigDcyFStIDcyFStMap.end())
        {
          if(m_iSigDcyFStICcDcyFStMap[m_iSigDcyTrISigDcyFStMap[m_vISigDcyTr[i]]] >= 0)
            fout << m_iSigDcyFStIDcyFStMap[m_iSigDcyTrISigDcyFStMap[m_vISigDcyTr[i]]];
          else
            fout << "$ " << m_iSigDcyFStIDcyFStMap[m_iSigDcyTrISigDcyFStMap[m_vISigDcyTr[i]]]
                 << "_{\\rm cc} $";
        }
      ...
      if(m_ccSwitch == true)
      {
        if(m_vICcSigDcyTr[i] == 0)
          fout << "---";
        else
          fout << m_vNCcSigDcyTr[i];
    ...
    for(unsigned int i = 0; i < m_vSigDcyFSt.size(); i++)
    {
      if(i % nLinesMax == 0)
      {
        ...
        fout << "\\begin{table}[htbp!]" << endl;
        if(i == 0)
          fout << "\\caption{Signal decay final states corresponding to signal decay trees.}"
               << endl;
        fout << "\\small" << endl;
        fout << "\\centering" << endl;
        fout << "\\begin{tabular}{|c|c|c|c|c|c|";
      ...
      if(m_sprTopoTags == false)
      {
        ...
        {
          if(m_iSigDcyFStICcDcyFStMap[m_vISigDcyFSt[i]] >= 0)
            fout << m_iSigDcyFStIDcyFStMap[m_vISigDcyFSt[i]];
          else
            fout << "$ " << m_iSigDcyFStIDcyFStMap[m_vISigDcyFSt[i]] << "_{\\rm cc} $";
        }
        else
          fout << "---";
        fout << " & ";
      ...
      if(m_ccSwitch == true)
      {
        if(m_vICcSigDcyFSt[i] == 0)
          fout << "---";
        else
          fout << m_vNCcSigDcyFSt[i];
  ...
  if(m_vSigDcyFSt2.size() > 0)
  {
    ...
    for(unsigned int i = 0; i < m_vSigDcyFSt2.size(); i++)
    {
      ...
      if(m_sprTopoTags == false)
      {
        ...
        {
          if(m_iSigDcyFSt2ICcDcyFStMap[m_vISigDcyFSt2[i]] >= 0)
            fout << m_iSigDcyFSt2IDcyFStMap[m_vISigDcyFSt2[i]];
          else
            fout << "$ " << m_iSigDcyFSt2IDcyFStMap[m_vISigDcyFSt2[i]] << "_{\\rm cc} $";
        }
        else
          fout << "---";
        fout << " & ";
      ...
      if(m_ccSwitch == true)
      {
        if(m_vICcSigDcyFSt2[i] == 0)
          fout << "---";
        else
          fout << m_vNCcSigDcyFSt2[i];
