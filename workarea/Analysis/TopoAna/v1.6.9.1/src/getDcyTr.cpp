                       ...
                       vector<int>& vIdxOfHead, vector<int>& vMidxOfHead)
{
  ...
  for(unsigned int i = 0; i < vPid.size(); i++)
  {
    if((((unsigned int)vMidx[i]) == i) && (vPid[i] != m_pidOfAnything) &&
       (vPid[i] != m_pidOfIRAFlag))
      dcyBr.push_back(vPid[i]);
  }
  dcyBr.push_front(11);
  dcyBr.push_front(-11);
  dcyTr.push_back(dcyBr);
  ...
  for(unsigned int i = 0; i < vPid.size(); i++)
  {
    for(unsigned int j = i + 1; j < vPid.size(); j++)
    {
      if((((unsigned int)vMidx[j]) == i) && (vPid[j] != m_pidOfAnything) &&
         (vPid[j] != m_pidOfIRAFlag))
        dcyBr.push_back(vPid[j]);
    }
    if(dcyBr.size() == 0) continue;
    if(abs(vPid[i]) != 1 && abs(vPid[i]) != 2 && abs(vPid[i]) != 3 && abs(vPid[i]) != 4 &&
       abs(vPid[i]) != 5 && abs(vPid[i]) != 6)
                       ...
                       vector<int>& vIIncIncOrIRASeqDcyBrs, vector<int>& vIIRAIncOrIRASeqDcyBrs)
{
  ...
  for(unsigned int i = 0; i < vPid.size(); i++)
  {
    ...
    {
      if(vPid[i] == m_pidOfAnything)
        iIncIncOrIRASeqDcyBrs = 1;
      else if(vPid[i] == m_pidOfIRAFlag)
        iIRAIncOrIRASeqDcyBrs = 1;
      else
        dcyBr.push_back(vPid[i]);
    }
  }
