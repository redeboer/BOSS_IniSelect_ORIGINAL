...
int main()
{
  ...
  for(unsigned int i = 0; i < vPid.size(); i++)
  {
    ...
    for(unsigned int j = 0; j < vPid_b2.size(); j++)
    {
      ...
      {
        cout << setiosflags(ios::right) << setw(16) << vPid[i] << setw(8) << v3Pchrg[i]
             << resetiosflags(ios::right) << "\t" << setiosflags(ios::left) << setw(24)
             << vTxtPnm[i] << endl;
        cout << setiosflags(ios::right) << setw(16) << vPid_b2[j] << setw(8) << v3Pchrg_b2[j]
             << resetiosflags(ios::right) << "\t" << setiosflags(ios::left) << setw(24)
             << vTxtPnm_b2[j] << setw(32) << vTexPnm_b2[j] << endl;
        cout << endl;
      }
    }
