...
unsigned int topoana::countIncSeqDcyBrsInDcyTr(vector<int> vIIncIncSeqDcyBrs, vector< list<int> > & incSeqDcyBrs, vector<int> vIdxOfHead1, vector<int> vMidxOfHead1, vector< list<int> > & dcyTr, vector<int> vIdxOfHead2, vector<int> vMidxOfHead2)
{
  unsigned int nCount=0;
  if(incSeqDcyBrs.size()==0)
    {
      cerr<<"Infor: The size of the vector for the inclusive sequential decay branches is zero!"<<endl<<endl;
      return nCount;
    }
  if(dcyTr.size()==0)
    {
              ...
              for(unsigned int k=vVIDcyBr2WRTIDcyBr1[i][j-1]+1;k<dcyTr.size();k++)
                {
                  if(vIIncIncSeqDcyBrs[j]==0)
                    {                  
                      if(dcyTr[k]==incSeqDcyBrs[j]&&(vMidxOfHead2[k]==vIdxOfHead2[k]||vMidxOfHead2[k]==vIdxOfHead2[(unsigned int) vVIDcyBr2WRTIDcyBr1[i][(unsigned int) vMIDcyBr1WRTIDcyBr1[j]]]))
                        {
                    ...
                    {
                      if(isLiaMatchedWithLib(incSeqDcyBrs[j],dcyTr[k])&&(vMidxOfHead2[k]==vIdxOfHead2[k]||vMidxOfHead2[k]==vIdxOfHead2[(unsigned int) vVIDcyBr2WRTIDcyBr1[i][(unsigned int) vMIDcyBr1WRTIDcyBr1[j]]]))
                        {
                          vVIDcyBr2WRTIDcyBr1[i].push_back(k);
                          break;
                        }
