...
void topoana::sortPs(vector<int> & vPid,vector<int> & vMidx)
{
  ...
  unsigned int hHdDcyBrs=1; // Codes related to the variable is used to contol maximum hierarchy of heading decay branches to be processed in each event. 
          ...
          for(unsigned int j=0;j<((unsigned int) vNSmPidsOld[i]);j++)
              ...
              for(unsigned int k=0;k<vPid.size();k++)
	        {
                  ...
                  if(((((unsigned int) vMidx[k])!=k)&&(vMidx[k]!=-1))&&(vMidx[k]==vIdxOld[nCmltSmPids+j]))
	            {
	              ...
	              vMidxYngSbst.push_back(vMidx[k]); // Note that vMidx[k] is equal to vIdxOld[nCmltSmPids+j].
