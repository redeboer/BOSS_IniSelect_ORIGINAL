...
void topoana::sortPs(vector<int>& vPid, vector<int>& vMidx)
{
  ...
  for(unsigned int i = 0; i < vPid.size(); i++)
  {
    ...
    if((((unsigned int)vMidx[i]) == i) || (vMidx[i] == -1))
    {
      ...
      else if(m_ignoreGISR == false)
        vPidOld.push_back(m_pidOfGISRGam);
      else
        continue;
      // vMidxOld.push_back(vMidx[i]); Sometimes, users may set Midx[i] at -1 rather than i when the
      // pid of the mother of the partilce i does not appear in the array Pid. Note that the arrays
      // Pid and Midx are the branches stored in the tree of the input root files. In order to make
      // the program can handle such cases as well, this statement is revised to be the following
      // one. Then, the following parts of the program do not have to change any more.
  ...
  unsigned int hHdDcyBrs = 1; // Codes related to the variable is used to contol maximum hierarchy
                              // of heading decay branches to be processed in each event.
  bool isIdPid;  // Codes related to the variable is used to ignore the decay of the particles
                 // specified in the input card and stored in the vector m_vIdPid.
  ...
  while(hHdDcyBrs < m_hHdDcyBrsMax && vIdxOld.size() != 0)
  {
    ...
    for(unsigned int i = 0; i < vNSmPidsOld.size(); i++)
    {
      ...
      {
        nCmltSmPids =
          nCmltSmPids + vNSmPidsOld[i]; // Don't miss out the statement. Otherwise, the program will
                                        // go wrong where the variable nCmltSmPids is used.
        continue;
      }
      ...
      for(unsigned int j = 0; j < ((unsigned int)vNSmPidsOld[i]); j++)
      {
        ...
        {
          // if((((unsigned int) vMidx[k])!=k)&&vMidx[k]==vIdxOld[nCmltSmPids+j]) Sometimes, users
          // may set Midx[i] at -1 rather than i when the pid of the mother of the partilce i does
          // not appear in the array Pid. Note that the arrays Pid and Midx are the branches stored
          // in the tree of the input root files. In order to make the program can handle such cases
          // as well, this statement is revised to be the following one.
          if(((((unsigned int)vMidx[k]) != k) && (vMidx[k] != -1)) &&
             (vMidx[k] == vIdxOld[nCmltSmPids + j]))
          {
            if(abs(vPid[k]) != 22) { vPidYngSbst.push_back(vPid[k]); }
            else if(vPid[k] == -22)
            {
              if(m_ignoreGFSR == false)
            ...
            {
              bool isGFSR = false;
              // In the following conditions, (k-l) is necessary to be converted from (unsigned int)
              // type to (int) type. Or, the condition will lose its effectiveness when k<l.
              for(unsigned int l = 1; ((k + l) < vPid.size() || ((int)(k - l)) >= 0); l++)
              {
                if((k + l) < vPid.size())
                {
                  ...
                  {
                    if(abs(vPid[k + l]) == 11 || abs(vPid[k + l]) == 13 ||
                       abs(vPid[k + l]) == 211 || abs(vPid[k + l]) == 321 ||
                       abs(vPid[k + l]) == 2212)
                    {
                      isGFSR = true;
                      break;
              ...
              {
                if(m_ignoreGFSR == false)
                  vPidYngSbst.push_back(m_pidOfGFSRGam);
                else
                  continue;
              }
              else
            ...
            vMidxYngSbst.push_back(
              vMidx[k]); // Note that vMidx[k] is equal to vIdxOld[nCmltSmPids+j].
        ...
        if(vPidYngSbst.size() > 0)
        {
          ...
          // The following four statements should be put in the scope of the if statement, otherwise
          // empty vectors might be pushed back to these vectors of vector.
      ...
      if(vVPidYngSbst.size() > 1)
        sortBySzPidAndPchrg(vVPidYngSbst, vVMidxYngSbst, vVIdxYngSbst, vVNSmPidsYngSbst);
  ...
  for(unsigned int i = 0; i < vPid.size(); i++)
    for(unsigned int j = i + 1; j < vPid.size(); j++)
    {
      if(vPid[j] == vPid[i] && vMidx[j] == vMidx[i])
      {
        ...
        if((vPidYngi.size() != 0) && (vPidYngi == vPidYngj))
          recurHighHierExchOrd(vIdxYngi, vIdxYngj, vPid, vMidx);
      }
    }
