...
void topoana::sortByPidAndPchrg(vector<int>& via, vector<int>& vib, vector<int>& vic,
                                vector<int>& vid)
{
  ...
  for(unsigned int i = 0; i < (via.size() - 1); i++)
    for(unsigned int j = i + 1; j < via.size(); j++)
    {
      ...
      {
        if(m_pid3PchrgMap.find(via[i]) != m_pid3PchrgMap.end() &&
           m_pid3PchrgMap.find(via[j]) != m_pid3PchrgMap.end())
        {
...
void topoana::sortByPidAndPchrg(list<int>& lia)
{
  ...
  for(liait1 = lia.begin(); liait1 != liaitLastButOne; liait1++)
    for(liait2 = liait1, liait2++; liait2 != lia.end(); liait2++)
    {
      ...
      {
        if(m_pid3PchrgMap.find((*liait1)) != m_pid3PchrgMap.end() &&
           m_pid3PchrgMap.find((*liait2)) != m_pid3PchrgMap.end())
        {
        ...
        {
          if(m_pid3PchrgMap.find((*liait1)) == m_pid3PchrgMap.end())
            writeErrInfOnPid3PchrgMap((*liait1));
          if(m_pid3PchrgMap.find((*liait2)) == m_pid3PchrgMap.end())
            writeErrInfOnPid3PchrgMap((*liait2));
        }
      }
      else
      {
