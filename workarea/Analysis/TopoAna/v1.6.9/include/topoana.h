...
private:
  ...
  // Pay attention to that m_vVDcyBrCcP, m_vVIDcyBrCcP and m_vVNDcyBrCcP equate to m_vVCcDcyBrP,
  // m_vVICcDcyBrP and m_vVNCcDcyBrP for self-charge-conjugate particles.
  ...
  // Pay attention to that m_vVDcyBrCcIncDcyBr, m_vVIDcyBrCcIncDcyBr and m_vVNDcyBrCcIncDcyBr equate
  // to m_vVCcDcyBrIncDcyBr, m_vVICcDcyBrIncDcyBr and m_vVNCcDcyBrIncDcyBr for self-charge-conjugate
  // inclusive decays.
...
public:
  topoana()
  {
    ...
    m_nEtrsMax =
      ULONG_MAX; // ULONG_MAX=9223372036854775807 is the maximum unsigned long int number.
    m_cutMethod               = false;
    m_ccSwitch                = false;
    m_ignoreGISR              = true;
    m_ignoreGFSR              = true;
  ...
  void    sortBySzPidAndPchrg(vector<vector<int> >& vVia, vector<vector<int> >& vVib,
                              vector<vector<int> >& vVic, vector<vector<int> >& vVid);
  bool    cmprBySzPidAndPchrg(vector<int>& via, vector<int>& vib);
  void    recurHigherHierExchOrd(vector<int>& vNewIdx, vector<int>& vOldIdx, vector<int>& vPid,
                                 vector<int>& vMidx);
  void    recurHighHierExchOrd(vector<int>& vIdxYngi, vector<int>& vIdxYngj, vector<int>& vPid,
                               vector<int>& vMidx);
  ...
  void    getDcyTr(vector<int> vPid, vector<int> vMidx, vector<list<int> >& dcyTr,
                   vector<int>& vIdxOfHead, vector<int>& vMidxOfHead, vector<int>& vIIncIncSeqDcyBrs);
  void    getDcyTr(vector<int> vPid, vector<int> vMidx, vector<list<int> >& dcyTr,
                   vector<int>& vIdxOfHead, vector<int>& vMidxOfHead,
  ...
  unsigned int countIRADcyBr(vector<int>& vPid, vector<int>& vMidx, list<int> IRADcyBr,
                             bool areHeadsRequiredToBeMatched = false);
  ...
  unsigned int countSeqDcyBrsInDcyTr(vector<list<int> >& seqDcyBrs, vector<int> vIdxOfHead1,
                                     vector<int> vMidxOfHead1, vector<list<int> >& dcyTr,
                                     vector<int> vIdxOfHead2, vector<int> vMidxOfHead2);
  unsigned int countIncSeqDcyBrsInDcyTr(vector<int>         vIIncIncSeqDcyBrs,
                                        vector<list<int> >& incSeqDcyBrs, vector<int> vIdxOfHead1,
                                        vector<int> vMidxOfHead1, vector<list<int> >& dcyTr,
                                        vector<int> vIdxOfHead2, vector<int> vMidxOfHead2);
  ...
  unsigned int countIncOrIRASeqDcyBrsInDcyTr(vector<int>         vIIncIncOrIRASeqDcyBrs,
                                             vector<int>         vIIRAIncOrIRASeqDcyBrs,
                                             vector<list<int> >& incOrIRASeqDcyBrs,
                                             vector<int> vIdxOfHead1, vector<int> vMidxOfHead1,
                                             vector<list<int> >& dcyTr, vector<int> vIdxOfHead2,
                                             vector<int> vMidxOfHead2, vector<int> vPid,
                                             vector<int> vMidx);
  void         sortBySumOf1stAnd2ndFromLrgToSml(vector<int>& via1, vector<int>& via2,
  ...
  void         sortBySumOf1stAnd2ndFromLrgToSml(vector<int>& via1, vector<int>& via2,
                                                vector<vector<list<int> > >& vVLib1,
                                                vector<vector<list<int> > >& vVLib2, vector<int>& vic1,
                                                vector<int>& vic2, vector<vector<int> >& vid1,
                                                vector<vector<int> >& vid2, vector<vector<int> >& vie1,
                                                vector<vector<int> >& vie2);
  void         sortBySumOf1stAnd2ndFromLrgToSml(vector<int>& via1, vector<int>& via2,
                                                vector<list<int> >& vLib1, vector<list<int> >& vLib2,
                                                vector<int>& vic1, vector<int>& vic2);
  void sortBySumOf1stAnd2ndFromLrgToSml(vector<int>& via1, vector<int>& via2, vector<int>& vib1,
                                        vector<int>& vib2, vector<int>& vic1, vector<int>& vic2);
  void sortBy1stFromLrgToSml(vector<int>& via, vector<vector<list<int> > >& vVLib,
                             vector<int>& vic);
  void sortBy1stFromLrgToSml(vector<int>& via, vector<vector<list<int> > >& vVLib, vector<int>& vic,
                             vector<vector<int> >& vVid, vector<vector<int> >& vVie);
