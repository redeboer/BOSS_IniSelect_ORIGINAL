...
int main()
{
  ...
  for(unsigned int i = 0; i < vPid.size(); i++)
  {
    if(vPid[i + 1] == -vPid[i])
    {
      fout << setiosflags(ios::right) << setw(16) << vPid[i] << setw(8) << v3Pchrg[i]
           << resetiosflags(ios::right) << "\t" << setiosflags(ios::left) << setw(24) << vTxtPnm[i]
           << setw(32) << vTexPnm[i] << setiosflags(ios::right) << setw(2) << 1 << endl;
      fout << setiosflags(ios::right) << setw(16) << vPid[i + 1] << setw(8) << v3Pchrg[i + 1]
           << resetiosflags(ios::right) << "\t" << setiosflags(ios::left) << setw(24)
           << vTxtPnm[i + 1] << setw(32) << vTexPnm[i + 1] << setiosflags(ios::right) << setw(2)
           << -1 << endl;
      i++;
    }
    else
    {
      fout << setiosflags(ios::right) << setw(16) << vPid[i] << setw(8) << v3Pchrg[i]
           << resetiosflags(ios::right) << "\t" << setiosflags(ios::left) << setw(24) << vTxtPnm[i]
           << setw(32) << vTexPnm[i] << setiosflags(ios::right) << setw(2) << 0 << endl;
    }
  }
  return 0;
}
