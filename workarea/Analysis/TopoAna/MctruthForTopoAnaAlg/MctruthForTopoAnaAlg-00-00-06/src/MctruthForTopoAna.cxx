...
StatusCode MctruthForTopoAna::execute()
{
  ...
  for(; iter_mc != mcParticleCol->end(); iter_mc++)
  {
    ...
    if(!incPdcy || (*iter_mc)->particleProperty() == incPid1 ||
       (*iter_mc)->particleProperty() == incPid2)
      continue;
    m_pid[m_nmcps] = (*iter_mc)->particleProperty();
    if(((*iter_mc)->mother()).particleProperty() == incPid1 ||
       ((*iter_mc)->mother()).particleProperty() == incPid2)
      m_midx[m_nmcps] = -1;
    else
