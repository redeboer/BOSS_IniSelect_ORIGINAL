...
/// Constructor for this class is emptry, but needs to be defined, because it inherits from
/// `Algorithm`.
...
StatusCode MctruthForTopoAna::initialize()
{
  ...
  /// <li> Book `NTuple` called `"MctruthForTopoAna"`. A selection of MC truth data will be written
  /// to this `TTree`. This tree contains:
...
StatusCode MctruthForTopoAna::execute()
{
  ...
  /// <li> Loop over collection of MC particle objects. An array containing a selection of MC truth
  /// particles is created here.
  ...
  for(; it != mcParticleCol->end(); ++it)
  {
    ...
    /// <li> The `rootIndex` variable is used to skip particles like the \f$Z\f$ boson in the decay
    /// chain after the \f$e^+e^-\f$ collision. It has to be stored once and untill it is stored,
    /// all MC truth particles are rejected (which is what the `incPdcy` boolean is for).
    if((*it)->particleProperty() == incPid1 || (*it)->particleProperty() == incPid2)
    {
      incPdcy   = true;
      rootIndex = (*it)->trackIndex();
    }
    /// <li> Skip if this particle is the initial cluster itself.
    if(!incPdcy || (*it)->particleProperty() == incPid1 || (*it)->particleProperty() == incPid2)
      continue;
    /// <li> Store PDG code of this MC particle.
    m_pid[m_nmcps] = (*it)->particleProperty();
    /// <li> Store a track index of the mother. This ID is `-1` if it is the initial cluster (e.g.
    /// \f$J/\psi\f$).
    if((*it)->mother().particleProperty() == incPid1 ||
       (*it)->mother().particleProperty() == incPid2)
      m_midx[m_nmcps] = -1;
    else
