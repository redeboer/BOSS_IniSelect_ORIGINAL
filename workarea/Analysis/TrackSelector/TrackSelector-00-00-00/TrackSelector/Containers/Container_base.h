...
/// for instance for defining derived containers containing properties and `NTuple`s. See [this page](https://besiii.gitbook.io/boss/the-boss-afterburner/initial/motivation#problems-with-booking-procedures) for the motivation.
...
public:
  Container_base(const std::string& name, const std::string& desc = "") :
    fName(name),
    fDescription(desc)
  {} ///< Only construct the name and description strings
  ...
  {
    return fName;
  } ///< Get a pointer to the `C` string (!) of the name.
  const std::string& Description() const
  {
    return fDescription;
  } ///< Get a pointer to the `C` string (!) of the description.
private:
