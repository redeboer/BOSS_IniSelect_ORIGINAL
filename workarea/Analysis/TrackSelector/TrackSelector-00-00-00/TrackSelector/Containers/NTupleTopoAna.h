...
public:
  ...
  NTuple::Item<int>
    index; ///< Number of MC particles stored for this event. This one is necessary for loading
           ///< following two items, because they are arrays. @remark It is called `index`, because
           ///< you use it as an index in the `WriteMcTruthForTopoAna` loop when filling the
           ///< `particle` and `mother` arrays. Of course, when you exit the loop, this index also
           ///< gives the number of items.
  NTuple::Array<int> particle; ///< PDG code for the particle in this array.
  NTuple::Array<int> mother; ///< Array index that points to which particle in the `particle` array
                             ///< is the mother particle. Is a `-1` if the mother is a cluster (not
