...
/// algorithm of `IniSelectObjects`, you should create a derived class of `KKFitResult` where you
...
public:
  ...
  /// @name Parameters
  ///@{
  double fChiSquared;  ///< \f$\chi^2\f$ of *this* fit (hence, not `static`).
...
protected:
  ...
  ///@{
  double fFitMeasure; ///< Measure that can be used to compare fits. @remark Usage depends
                      ///< completely on your implementation in the derived class.
  static double fBestCompareValue; ///< Current best value for the comparison. Note that it is
                                   ///< automatically reset to the value set here when all (derived)
                                   ///< instances of `KKFitResult` are destroyed.
  KalmanKinematicFit* fFit;
  bool fHasResults; ///< You will only want to write the fit results if they have been properly
                    ///< computed. This bit is set to `true` if either this object has been
                    ///< constructed using a non-empty `KalmanKinematicFit` pointer or has been
                    ///< constructed with (see derived classes for implementation)
  ///@}
};
