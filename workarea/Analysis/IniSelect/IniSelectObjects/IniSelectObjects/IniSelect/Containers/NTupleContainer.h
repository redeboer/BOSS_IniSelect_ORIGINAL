#ifndef Analysis_IniSelect_NTupleContainer_H
#define Analysis_IniSelect_NTupleContainer_H

#include "GaudiKernel/NTuple.h"
#include "IniSelectObjects/Containers/Container_base.h"
#include "IniSelectObjects/Containers/JobSwitch.h"
#include <iostream>
#include <map>
#include <string>
...
/// @remark This object can only be used within a derived class off the Gaudi `Algorithm` class because its `fTuple` requires to be booked by it (using e.g. `IniSelectObjects::BookNTuples`).
...
public:
  ...
  ///@{
  template <typename TYPE>
  inline NTuple::Item<TYPE>& GetItem(const std::string& key);
  template <typename TYPE>
  inline NTuple::Array<TYPE>& GetArray(const std::string& key);
  ...
  ///@{
  template <typename TYPE>
  inline NTuple::Array<TYPE>* AddIndexedItem(const std::string& item_name,
                                             NTuple::Item<int>& index);
  template <typename TYPE>
  inline NTuple::Item<TYPE>* AddItem(const std::string& item_name);
  NTuple::Item<double>*      AddItem(const std::string& item_name)
  {
    return AddItem<double>(item_name);
  }; ///< Shortcut for `AddItem<double>`, added for backward compatibilaty.
  template <typename TYPE, typename RANGE>
  inline NTuple::Item<TYPE>* AddItem(const std::string& item_name, const RANGE low,
                                     const RANGE high);
  void                       Write();
  ///@}
  ...
  ///@{
  const bool DoWrite() const
  {
    return (bool)fWrite;
  } /// Return `true` if `fTuple` pointer exists and if `fWrite` property has been set to `true`.
...
/// Method that is comparable to `std::map::at`, but allows to access the diffent types of maps
/// (`fItems_double` etc) through a template call of this method. Call e.g. using
/// `GetArray<double>("fit4c")`.
template <typename TYPE>
inline NTuple::Array<TYPE>& NTupleContainer::GetArray(const std::string& key)
{
  try
  {
...
/// Method that is comparable to `std::map::at`, but allows to access the diffent types of maps
/// (`fItems_double` etc) through a template call of this method. Call e.g. using
/// `GetItem<double>("fit4c")`.
template <typename TYPE>
inline NTuple::Item<TYPE>& NTupleContainer::GetItem(const std::string& key)
{
  try
  {
...
TYPE* NTupleContainer::AddToMap(std::map<std::string, TYPE>* map, const std::string& item_name)
{
  ...
  if(!fTuple)
  {
    std::cout << "FATAL ERROR: NTuple \"" << Name()
              << "\" has not been booked, so cannot add item \"" << item_name << "\"" << std::endl;
...
/// Easier and expanded version of `NTuple::Tuple::addIndexedItem` that also adds the item to the
/// `fArrays_*` mappings. You will need to call this method using e.g. `AddIndexedItem<double>` to
/// specify that this is an array of `double`s.
template <typename TYPE>
inline NTuple::Array<TYPE>* NTupleContainer::AddIndexedItem(const std::string& item_name,
                                                            NTuple::Item<int>& index)
{
...
/// Easier and expanded version of `NTuple::Tuple::addItem` that also adds the item to the
/// `fItems_*` mappings. You will need to call this method using e.g. `AddItem<double>` to specify
/// that this is a `double`.
template <typename TYPE>
inline NTuple::Item<TYPE>* NTupleContainer::AddItem(const std::string& item_name)
{
  /// -# Attempt to create an item in the item or array mapping using `AddToMap`.
...
template <typename TYPE, typename RANGE>
inline NTuple::Item<TYPE>* NTupleContainer::AddItem(const std::string& item_name, const RANGE low,
                                                    const RANGE high)
{
  /// -# Attempt to create an item in the item or array mapping using `AddToMap`.
  NTuple::Item<TYPE>* item = AddToMap(GetItems<TYPE>(), item_name);
