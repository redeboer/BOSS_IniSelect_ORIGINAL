#ifndef Analysis_IniSelect_Container_indexed_H
#define Analysis_IniSelect_Container_indexed_H
...
protected:
  ...
  template <class TYPE>
  inline void Insert(std::map<std::string, TYPE*>& mapping, const std::string& key, TYPE* value);
...
template <class TYPE>
inline bool Container_indexed::KeyExists(const std::map<std::string, TYPE*> mapping,
...
/// Helper method that allows you to safely (using `KeyExists`) insert a `key` with corresponding
/// `value` into a `mapping`. This method uses a `template` and has been made `protected` as to
/// share its functionality for derived classes.
template <class TYPE>
inline void Container_indexed::Insert(std::map<std::string, TYPE*>& mapping, const std::string& key,
                                      TYPE* value)
{
