...
//**************************************************************************************
DTag::DTag(const std::string& name, ISvcLocator* pSvcLocator) : Algorithm(name, pSvcLocator)
{
...
StatusCode DTag::initialize()
{
  ...
  if(m_recD0)
  {
    sc = createSubAlgorithm("NeutralDReconstruction", "NeutralDReconstruction",
                            m_NeutralDReconstruction);
    if(sc.isFailure())
    {
  ...
  if(m_recDp)
  {
    sc = createSubAlgorithm("ChargedDReconstruction", "ChargedDReconstruction",
                            m_ChargedDReconstruction);
    if(sc.isFailure())
    {
...
StatusCode DTag::execute()
{
  ...
  DataObject* aEvtRecDTagCol;
  eventSvc()->findObject("/Event/EvtRec/EvtRecDTagCol", aEvtRecDTagCol);
  if(aEvtRecDTagCol == NULL) { registerEvtRecDTagCol(log); }
  ...
  {
    sc = (*it)->execute();
    if(sc.isFailure()) { log << "Error executing selection " << (*it)->name() << endreq; }
  }
