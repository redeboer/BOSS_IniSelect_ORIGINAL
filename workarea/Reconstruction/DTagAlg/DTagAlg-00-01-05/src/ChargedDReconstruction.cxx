...
StatusCode ChargedDReconstruction::registerEvtRecDTagCol(EvtRecDTagCol* aNewEvtRecDTagCol,
                                                         MsgStream&     log)
{
  ...
  */
  ...
  for(int list = 0; list < chanlist.size(); list++)
  {
    ...
    else if(channel == "DptoKPiPiPiPi")
    {
      ...
      numchan.push_back(2);
      decaylist =
        kaonList.minus() * pionList.plus() * pionList.plus() * pionList.plus() * pionList.minus();
    }
...
void ChargedDReconstruction::saveDpInfo(CDDecayList::iterator it, double ebeam, int numofchildren,
                                        EvtRecDTag* recDTag)
{
...
void ChargedDReconstruction::savetrack(vector<int> trackid, vector<int> showerid,
...
void ChargedDReconstruction::pidtag(vector<int> kaonid, vector<int> pionid,
                                    CDChargedKaonList& kaonList, CDChargedPionList& pionList,
                                    EvtRecDTag* recDTag)
{
