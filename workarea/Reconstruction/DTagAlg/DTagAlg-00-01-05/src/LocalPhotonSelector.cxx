...
bool LocalPhotonSelector::operator()(CDPhoton& aPhoton)
{
  ...
  HepLorentzVector shP4(eraw * sin(the) * cos(phi), eraw * sin(the) * sin(phi), eraw * cos(the),
                        eraw);
  ...
  if(m_useBarrelEndcap)
  {
    ...
    if((fabs(cosThetaSh) > m_minCosThetaEndcap) && (fabs(cosThetaSh) < m_maxCosThetaEndcap) &&
