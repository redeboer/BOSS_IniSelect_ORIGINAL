...
DsSelector::DsSelector()
{
  ...
  m_propMgr.declareProperty("UseDeltaECuts", m_useDeltaECuts = false);
  m_propMgr.declareProperty("UseDeltaMassCuts", m_useDeltaMassCuts = true);

  m_propMgr.declareProperty("DMbcMinCut", m_minMbc = 1.93);
  ...
  jobSvc->setMyProperties("DsSelector", &m_propMgr);
}
...
bool DsSelector::operator()(CDDecay& theD)
{
  ...
  double chargedDMass = 1.9685;
  double deltaMass    = theD.mass() - chargedDMass;

  if(m_useDeltaMassCuts && ((deltaMass <= m_minDeltaMass) || (deltaMass >= m_maxDeltaMass)))
    return false;
