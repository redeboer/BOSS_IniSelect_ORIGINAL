...
StatusCode DsReconstruction::registerEvtRecDTagCol(EvtRecDTagCol* aNewEvtRecDTagCol, MsgStream& log)
{
  StatusCode sc = eventSvc()->registerObject("/Event/EvtRec/EvtRecDTagCol", aNewEvtRecDTagCol);
  if(sc != StatusCode::SUCCESS)
  { log << MSG::FATAL << "Could not register EvtRecDTagCol in TDS!" << endreq; } return sc;
}
...
StatusCode DsReconstruction::execute()
{
  ...
  log << MSG::DEBUG << "ncharg, nneu, tottks = " << recEvent->totalCharged() << " , "
      << recEvent->totalNeutral() << " , " << recEvent->totalTracks() << endreq;
  ...
  */
  ...
  for(int list = 0; list < chanlist.size(); list++)
  {
    ...
    else if(channel == "DstoKKPiPiPi")
    {
      ...
      numchan.push_back(2);
      decaylist =
        kaonList.minus() * kaonList.plus() * pionList.plus() * pionList.plus() * pionList.minus();
    }
    ...
    else if(channel == "DstoPiPiPiPiPi")
    {
      ...
      numchan.push_back(2);
      decaylist =
        pionList.plus() * pionList.plus() * pionList.plus() * pionList.minus() * pionList.minus();
    }
    else if(channel == "DstoPiPiPiPiPiPi0")
    {
      ...
      numchan.push_back(2);
      numchan.push_back(3);
      decaylist = pionList.plus() * pionList.plus() * pionList.plus() * pionList.minus() *
                  pionList.minus() * pi0List;
    }
    else if(channel == "DstoPiPiPiPi0Pi0")
    { // <------------- NEW MODE
    ...
    else if(channel == "DstoPiEtaPiPiPi0")
    { // <------------- NEW MODE
    ...
    else if(channel == "DstoPiPi0EtaPiPiPi0")
    { // <---------- NEW MODE
    ...
    else if(channel == "DstoPiPiPiEtaPiPiPi0")
    { // <---------- NEW MODE
    ...
    else if(channel == "DstoPiEPPiPiEtaPiPiPi0")
    { // <------------- New mode: 470 eta'(pipieta, eta->pipipi0)
    ...
    else if(channel == "DstoPiPi0EPPiPiEtaPiPiPi0")
    { //<------------ New mode: 471 eta'(pipieta, eta->pipipi0)
    ...
    for(CDDecayList::iterator it = D_begin; it != D_end; it++)
    {
      ...
      for(int i = 0; i < numofchildren; i++)
      {
        ...
        { // <------------ NEW PART FOR ETAP TO PIPIETA TO PIPIPI0
          const CDCandidate& apion  = daughter.decay().child(0);
          const CDCandidate& spion  = daughter.decay().child(1);
          const CDCandidate& eta    = daughter.decay().child(2);
...
void DsReconstruction::saveDsInfo(CDDecayList::iterator it, double ebeam, int numofchildren,
                                  EvtRecDTag* recDTag)
{
...
void DsReconstruction::savetrack(vector<int> trackid, vector<int> showerid,
                                 EvtRecTrackIterator charged_begin, EvtRecTrackIterator charged_end,
                                 EvtRecTrackIterator neutral_begin, EvtRecTrackIterator neutral_end,
                                 EvtRecDTag* recDTag)
{
...
void DsReconstruction::pidtag(vector<int> kaonid, vector<int> pionid, CDChargedKaonList& kaonList,
                              CDChargedPionList& pionList, EvtRecDTag* recDTag)
{
  ...
  for(CDChargedKaonList::iterator kit = kaonList.particle_begin(); kit != kaonList.particle_end();
      kit++)
