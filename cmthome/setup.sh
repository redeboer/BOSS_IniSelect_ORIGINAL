...
${CMTROOT}/mgr/cmt setup -sh -pack=cmt_standalone -version=v0 -path=${BOSSWORKAREA}/cmthome  -no_cleanup $* >${cmtcmt_standalonetempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/mgr/cmt setup -sh -pack=cmt_standalone -version=v0 -path=${BOSSWORKAREA}/cmthome  -no_cleanup $* >${cmtcmt_standalonetempfile}"
  cmtsetupstatus=2
