...
void topoana::readCard(string topoAnaCardName)
{
  ...
  while(!infile.eof())      
    {
      ...
      if(line.empty()||line[0]=='#'||line[0]==' ')
      ...
      else if(line=="Begin input root file names")
        {
          ...
          if(line.empty()||line[0]=='#'||line[0]==' '||line=="End input root file names"||line.substr(0,5)=="Begin")
            {
              cerr<<"Error : There are not root file names after the prompt Begin input root file names !"<<endl; 
              exit(111);
            }
      ...
      else if(line=="Begin input tree name")
        {
          ...
          if(line.empty()||line[0]=='#'||line[0]==' '||line=="End input tree name"||line.substr(0,5)=="Begin")
            {
      ...
      else if(line=="Begin output main file name")
        {
          ...
          if(line.empty()||line[0]=='#'||line[0]==' '||line=="End output main file name"||line.substr(0,5)=="Begin")
            {
              cerr<<"Error : There is not main file name after the prompt Begin output main file name !"<<endl;
              exit(115);
            }
      ...
      else if(line=="Begin sigal particle identifications")
        {
          ...
          if(line.empty()||line[0]=='#'||line[0]==' '||line=="End sigal particle identifications"||line.substr(0,5)=="Begin")
            {
              cerr<<"Error : There are not sigal particle identifications after the prompt Begin sigal particle identifications !"<<endl; 
              exit(117);
            }
          iss.str(line);
          iss>>sigPid;
      ...
      else if(line=="Begin signal event decay trees")
        {
          ...
          if(line.empty()||line[0]=='#'||line[0]==' '||line=="End signal event decay trees"||line.substr(0,5)=="Begin")
            {
              cerr<<"Error : There are not signal event decay trees after the prompt Begin signal event decay trees !"<<endl; 
              exit(119);
            }
          iss.str(line);
  ...
  if(m_inputRootFileNames.size()==0)
  ...
  else
    {
      ...
      for(unsigned int i=0;i<m_inputRootFileNames.size();i++) cout<<m_inputRootFileNames[i]<<endl;  
      cout<<endl;
    }
  ...
  if(m_vSigPid.size()==0)
    {
      cerr<<"Info : there are not sigal particle identifications ."<<endl;
    }
  else
    {
      cout<<"m_vSigPid :"<<endl;
  ...
  if(m_vVSigPid.size()==0)
    {
      cerr<<"Info : there are not signal event decay trees ."<<endl;
    }
  else
    {
      ...
      for(unsigned int i=0;i<m_vVSigPid.size();i++)
	{
	    ...
	    {
	      cout<<j<<"\t"<<m_vVSigPid[i][j]<<"\t"<<m_vVSigMidx[i][j]<<endl;
	    }
          cout<<endl;
	}
      cout<<endl;
...
void topoana::evtReorder(vector<int> & vPid,vector<int> & vMidx)
{
  ...
  vector<int> vPidFinal,vMidxFinal,vIdxFinal,vPidTemp,vMidxTemp,vIdxTempOld,vIdxTempYoung,vIdxTempYoungSubset;
  for(unsigned int i=0;i<vPid.size();i++)
    {
      ...
      for(unsigned int i=0;i<vIdxTempOld.size();i++)
          ...
          for(unsigned int j=0;j<vPid.size();j++)
	    {
	      //if(((unsigned int) vMidx[j])!=j&&vMidx[j]==vIdxTempOld[i])
              if(((unsigned int) vMidx[j])!=j&&vPid[((unsigned int) vMidx[j])]!=111&&vMidx[j]==vIdxTempOld[i])
	        {
      ...
      for(unsigned int i=0;i<vPid.size()-2;i++)
        {
          for(unsigned int j=i+1;j<vPid.size()-1;j++)
            {
              if((vPid[i]==vPid[j])&&((((unsigned int) vMidx[i])==i&&((unsigned int) vMidx[j])==j)||vMidx[i]==vMidx[j]))
                {
                  vector<int> vIdxofdsofi,vIdxofdsofj;
                  for(unsigned int k=j+1;k<vPid.size();k++)
                    {
                      if(((unsigned int) vMidx[k])==i) vIdxofdsofi.push_back(k);
                      else if(((unsigned int) vMidx[k])==j) vIdxofdsofj.push_back(k);
                    }
                  bool flag=false;
                  if(vIdxofdsofi.size()>vIdxofdsofj.size()) flag=true;
                  ...
                  if(flag==true)
                    {
                      for(unsigned int m=0;m<vIdxofdsofi.size();m++)
                        {
                          vMidx[vIdxofdsofi[m]]=j;
                        }
                      for(unsigned int n=0;n<vIdxofdsofj.size();n++)
                        {
                          vMidx[vIdxofdsofj[n]]=i;
                        }
                    }
                }
            }
  ...
  for(unsigned int i=0;i<vPid.size();i++)
    {
      ...
      if(fsp&&abs(vPid[i])!=1&&abs(vPid[i])!=2&&abs(vPid[i])!=3&&abs(vPid[i])!=4&&abs(vPid[i])!=5&&abs(vPid[i])!=6) evtDcyIFChn.push_back(vPid[i]);
    }
...
void topoana::sortByFirstFromLargeToSmall(vector<int> &via,vector< vector< list<int> > > &vib,vector<int> &vic)
{
  if(via.size()!=vib.size()||vib.size()!=vic.size())
    {
      cerr<<"Error: the three vectors have different size!"<<endl;
  ...
  int iatemp;vector< list<int> > ibtemp;int ictemp;
  for(unsigned int i=0;i<via.size()-1;i++)
    for(unsigned int j=i+1;j<via.size();j++)
      if(via[i]<via[j])
        {
...
void topoana::sortByFirstFromLargeToSmall(vector<int> &via,vector< list<int> > &vib,vector<int> &vic)
{
  if(via.size()!=vib.size()||vib.size()!=vic.size())
    {
      cerr<<"Error: the three vectors have different size!"<<endl;
  ...
  int iatemp;list<int> ibtemp;int ictemp;
  for(unsigned int i=0;i<via.size()-1;i++)
    for(unsigned int j=i+1;j<via.size();j++)
      if(via[i]<via[j])
        {
...
void topoana::getResult()
{
  ...
  for(unsigned int i=0;i<m_inputRootFileNames.size();i++)
    {
      chain->Add(m_inputRootFileNames[i].c_str());
    }

  Int_t Nmcps,Pid[50],Midx[50];
  ...
  if(m_vSigPid.size()>0)
    {
      tree->Branch("NSigPid",&nSigPid,"nSigPid[50]/I");
    }
  if(m_vVSigPid.size()>0)
    {
          ...
          for(unsigned int j=0;j<m_vSigEvtDcyTr.size();j++)
            {
              if(sigEvtDcyTr==m_vSigEvtDcyTr[j])
                {
                  cerr<<"Error : The "<<i+1<<" signal event decay tree is same as the "<<j+1<<" signal event decay tree ! Please check the input card and remove one of them !"<<endl;
                  exit(512);    
                }
            }
          ...
          for(unsigned int j=0;j<m_vSigEvtDcyIFChn.size();j++)
            {
              if(sigEvtDcyIFChn==m_vSigEvtDcyIFChn[j])
                {
                  cout<<"Info : the final states of the "<<i+1<<" signal event decay tree is same as those of one of former event decay trees ."<<endl;
  ...
  if(m_vSigPid.size()>0)
    {  
      sortByFirstFromLargeToSmall(m_vNSigPid,m_vSigPid,m_vISigPid);
    }
  if(m_vVSigPid.size()>0)
    {
...
void topoana::outputResultIntoTxtFile()
{
  ...
  for(unsigned int i=0;i<m_vNEvtDcyTr.size();i++)
    { 
      ...
      outfile<<"index:  "<<i+1<<"\tiEvtDcyTr:  "<<m_vIEvtDcyTr[i]<<"\tnevents:  "<<m_vNEvtDcyTr[i]<<"\tnaccdevents:  "<<naccdevents<<endl;
      for(unsigned int j=0;j<evtDcyTr.size();j++)
        {
          ...
          outfile<<" --> ";
          for(liit++;liit!=evtDcyChn.end();liit++)
            {
              outfile<<m_pidTxtpnameMap[*liit]<<" ";
            }
          outfile<<endl;
  ...
  for(unsigned int i=0;i<m_vNEvtDcyIFChn.size();i++)
    {
      ...
      outfile<<"index:  "<<i+1<<"\tiEvtDcyIFChn:  "<<m_vIEvtDcyIFChn[i]<<"\tnevents:  "<<m_vNEvtDcyIFChn[i]<<"\tnaccdevents:  "<<naccdevents<<endl;
      list<int>::iterator liit=evtDcyIFChn.begin();
      outfile<<"  "<<m_pidTxtpnameMap[*liit];
      liit++;
      outfile<<" "<<m_pidTxtpnameMap[*liit]<<" --> ";
      for(liit++;liit!=evtDcyIFChn.end();liit++)
        {
          outfile<<m_pidTxtpnameMap[*liit]<<" ";
        }
      outfile<<endl<<endl;
      ...
      for(unsigned int i=0;i<m_vNSigPid.size();i++)
        {
          ...
          outfile<<"index:  "<<i+1<<"\tiSigPid:  "<<m_vISigPid[i]<<"\tSigName:  "<<m_pidTxtpnameMap[m_vSigPid[i]]<<"\tnevnets:   "<<m_vNSigPid[i]<<"\tnaccdevents:  "<<naccdevents<<endl<<endl;
        }
      outfile<<endl<<endl<<endl;
    }
  ...
  if(m_vVSigPid.size()>0)
    {
      ...
      for(unsigned int i=0;i<m_vNSigEvtDcyTr.size();i++)
        { 
          ...
          outfile<<"index:  "<<i+1<<"\tiSigEvtDcyTr:  "<<m_vISigEvtDcyTr[i]<<"\tnevents:  "<<m_vNSigEvtDcyTr[i]<<"\tnaccdevents:  "<<naccdevents<<endl;
      ...
      for(unsigned int i=0;i<m_vNSigEvtDcyIFChn.size();i++)
        {
          ...
          outfile<<"index:  "<<i+1<<"\tiSigEvtDcyIFChn:  "<<m_vISigEvtDcyIFChn[i]<<"\tnevents:  "<<m_vNSigEvtDcyIFChn[i]<<"\tnaccdevents:  "<<naccdevents<<endl;
  ...
  for(unsigned int i=0;i<m_vNEvtDcyTr.size();i++)
    {
      ...
      for(unsigned int j=0;j<evtDcyTr.size();j++)
        {
          ...
          evtDcyChn=evtDcyTr[j];
          list<int>::iterator liit=evtDcyChn.begin();
          if(((unsigned int) (*liit))==111) continue;
          else nVldEvtDcyChn++;
      ...
      outfile<<"$ & "<<m_vIEvtDcyTr[i]<<" & "<<m_vNEvtDcyTr[i]<<" & "<<naccdevents<<" \\\\"<<endl;
      outfile<<"\\hline"<<endl;
  ...
  for(unsigned int i=0;i<m_vNEvtDcyIFChn.size();i++)
    {
      if(i%35==0)
        {
          ...
          outfile<<"\\hline"<<endl;
          outfile<<"index & event decay initial-final chain & iEvtDcyIFChn & nevents & naccdevents \\\\"<<endl;
          outfile<<"\\hline"<<endl;
        }
      outfile<<i+1<<" & $"; 
      evtDcyIFChn.clear();
      ...
      outfile<<"$ & "<<m_vIEvtDcyIFChn[i]<<" & "<<m_vNEvtDcyIFChn[i]<<" & "<<naccdevents<<" \\\\"<<endl;
      outfile<<"\\hline"<<endl;
      ...
      for(unsigned int i=0;i<m_vNSigPid.size();i++)
        {
          ...
          naccdevents=naccdevents+m_vNSigPid[i];
          outfile<<i+1<<" & $"<<m_pidTexpnameMap[m_vSigPid[i]]<<" "<<"$ & "<<m_vISigPid[i]<<" & "<<m_vNSigPid[i]<<" & "<<naccdevents<<" \\\\"<<endl;
          outfile<<"\\hline"<<endl;
      ...
      for(unsigned int i=0;i<m_vNSigEvtDcyTr.size();i++)
        {
          if(i%35==0)
            {
              ...
              outfile<<"\\begin{tabular}{|c|>{\\centering\\arraybackslash}p{20cm}|c|c|c|}"<<endl;
              outfile<<"\\hline"<<endl;
              outfile<<"index & signal event decay trees & iSigEvtDcyTr & nevents & naccdevents \\\\"<<endl;
              outfile<<"\\hline"<<endl;
            }
          outfile<<i+1<<" & $"; 
          sigEvtDcyTr.clear();
          ...
          outfile<<"$ & "<<m_vISigEvtDcyTr[i]<<" & "<<m_vNSigEvtDcyTr[i]<<" & "<<naccdevents<<" \\\\"<<endl;
          outfile<<"\\hline"<<endl;
      ...
      for(unsigned int i=0;i<m_vNSigEvtDcyIFChn.size();i++)
        {
          if(i%35==0)
            {
              ...
              outfile<<"\\begin{tabular}{|c|c|c|c|c|}"<<endl;
              outfile<<"\\hline"<<endl;
              outfile<<"index & signal event decay initial-final chain & iSigEvtDcyIFChn & nevents & naccdevents \\\\"<<endl;
              outfile<<"\\hline"<<endl;
            }
          outfile<<i+1<<" & $"; 
          sigEvtDcyIFChn.clear();
          ...
          outfile<<"$ & "<<m_vISigEvtDcyIFChn[i]<<" & "<<m_vNSigEvtDcyIFChn[i]<<" & "<<naccdevents<<" \\\\"<<endl;
          outfile<<"\\hline"<<endl;
